...
// <<< ADDED: Gemini AI Model Initialization for Instagram Endpoint >>>
let geminiModelInstance;
try {
  const geminiApiKey = process.env.GEMINI_API_KEY;
  if (!geminiApiKey) {
    throw new Error(
      "GEMINI_API_KEY environment variable is not defined for server.js."
    );
  }
  const genAI = new GoogleGenerativeAI(geminiApiKey);
  geminiModelInstance = genAI.getGenerativeModel({ model: "gemini-1.5-flash" }); // Or your preferred model
  console.log(
    `[Server] Gemini Model for Instagram endpoint initialized successfully.`
  );
} catch (geminiInitError) {
  console.error(
    `[Server][ERROR CRÍTICO] Initializing GoogleGenerativeAI for Instagram endpoint:`,
    geminiInitError
  );
  // Decide if server should exit or continue without this functionality
  // process.exit(1);
}
// <<< END: Gemini AI Model Initialization >>>
...
try {
  ...
  if (!fs.existsSync(serviceAccountPath)) {
    throw new Error(
      `Archivo de clave de servicio no encontrado en: ${serviceAccountPath}`
    );
  }
  admin.initializeApp({
  ...
  console.log(
    "Firebase Admin SDK inicializado correctamente. Conectado a Firestore."
  );
} catch (error) {
...
console.log("==================================================");
console.log(
  `INICIANDO API PRINCIPAL (v2) EN PUERTO ${port} - ${new Date().toLocaleTimeString()}`
);
console.log("==================================================");
...
app.use((req, res, next) => {
  ...
  if (req.url.startsWith("/setter-api")) {
    ...
    console.log(
      `[Server] Ruta con prefijo /setter-api detectada. Redirigiendo a: ${req.url}`
    );
  }
  next();
});
...
if (!API_SECRET_KEY) {
  console.warn(
    "[SECURITY WARNING] API_SECRET_KEY environment variable is not set. API endpoints are unprotected!"
  );
}
...
wss.on("connection", (ws, req) => {
  console.log(
    `[Server][WebSocket][DEBUG] Incoming connection request. URL: ${req.url}`
  ); // Log raw URL
  ...
  try {
    ...
    if (urlParts.length > 1) {
      ...
      console.log(
        `[Server][WebSocket][DEBUG] Extracted userId: ${userId} from query params.`
      );
    } else {
      console.log(
        `[Server][WebSocket][DEBUG] No query parameters found in URL: ${req.url}`
      );
    }
  } catch (e) {
    console.error(
      `[Server][WebSocket][ERROR] Failed to parse URL or extract userId from ${req.url}:`,
      e
    );
  }
  ...
  if (!userId) {
    console.log(
      "[Server][WebSocket] Connection attempt without valid userId. Closing."
    );
  ...
  console.log(
    `[Server][WebSocket][DEBUG] Attempting to register client for userId: ${userId}`
  );
  console.log(`[Server][WebSocket] Client connected for user: ${userId}`);
  // Check if a client already exists for this user
  if (wsClients.has(userId)) {
    console.warn(
      `[Server][WebSocket][WARN] Overwriting existing WebSocket connection for user: ${userId}. Possible stale connection.`
    );
  ...
  console.log(
    `[Server][WebSocket][DEBUG] Client registered in wsClients map for userId: ${userId}. Map size: ${wsClients.size}`
  );

  ws.on("message", (message) => {
    // Handle messages from client if needed (e.g., ping/pong, specific commands)
    console.log(
      `[Server][WebSocket] Received message from ${userId}: ${message}`
    );
    ...
    try {
      ...
      if (parsedMessage.type === "PING") {
        if (DEBUG)
          console.log(
            `[Server][WebSocket] Received PING from ${userId}. Sending PONG.`
          );
  ...
  ws.on("close", (code, reason) => {
    ...
    console.log(
      `[Server][WebSocket] Client disconnected for user: ${userId}. Code: ${code}, Reason: ${reasonString}`
    );
    // Ensure the correct client is deleted if overwritten
    if (wsClients.get(userId) === ws) {
      wsClients.delete(userId);
      console.log(
        `[Server][WebSocket][DEBUG] Client removed from wsClients map for userId: ${userId}. Map size: ${wsClients.size}`
      );
    } else {
      console.warn(
        `[Server][WebSocket][WARN] Close event for user ${userId}, but the stored client was different. Not deleting from map.`
      );
    }
  });
  ...
  ws.on("error", (error) => {
    ...
    if (wsClients.get(userId) === ws) {
      ...
      console.log(
        `[Server][WebSocket][DEBUG] Client removed from wsClients map due to error for userId: ${userId}. Map size: ${wsClients.size}`
      );
    } else {
      console.warn(
        `[Server][WebSocket][WARN] Error event for user ${userId}, but the stored client was different. Not deleting from map.`
      );
    }
  });
...
app.use((req, res, next) => {
  console.log(
    `[API v2] ${req.method} ${req.url} - ${new Date().toLocaleTimeString()}`
  );
  next();
});
...
function notifyWorker(userId, message) {
  console.log(
    `[Server] notifyWorker a ${userId}. Mensaje tipo: ${
  ...
  if (!workerProcess) {
    console.error(
      `[Server][ERROR] No se puede enviar mensaje a worker ${userId}: no existe en workerProcesses`
    );
    return false;
  }
  ...
  try {
    console.log(
      `[Server] ENVIANDO mensaje a worker ${userId}:`,
      JSON.stringify(message)
    );
    const result = workerProcess.send(message);
    console.log(
      `[Server] Resultado de workerProcess.send para ${userId}: ${result}`
    );
    return result;
  } catch (error) {
    console.error(
      `[Server][ERROR CRÍTICO] Error enviando mensaje a worker ${userId}:`,
      error
    );
    return false;
  }
...
async function startWorker(userId) {
  ...
  if (connectingUsers.has(userId)) {
    console.warn(
      `[Server][CRITICAL] Intento de inicio concurrente para ${userId} bloqueado (ya está en proceso).`
    );
  ...
  console.log(
    `[Server][DEBUG] Locking connection attempt for ${userId}. connectingUsers:`,
    connectingUsers
  );
  // <<< END ADDED >>>
  ...
  try {
    ...
    if (workers[userId] && workers[userId].connected) {
      // <<< Check .connected explicitly >>>
      console.warn(
        `[Server][CRITICAL] Ya existe un worker CONECTADO para ${userId}. No se iniciará uno nuevo.`
      );
      // <<< MODIFIED: Release lock and return existing worker >>>
      connectingUsers.delete(userId);
      console.log(
        `[Server][DEBUG] Releasing lock (worker already exists) for ${userId}. connectingUsers:`,
        connectingUsers
      );
    ...
    if (workers[userId] && !workers[userId].connected) {
      console.warn(
        `[Server][CRITICAL] Worker para ${userId} existe pero no está conectado. Intentando limpiar y reiniciar.`
      );
      try {
        workers[userId].kill();
      } catch (e) {
        console.error("Error matando worker zombie:", e);
      }
    ...
    if (!fs.existsSync(sessionPath)) {
      ...
      console.log(
        `[Server] Creado directorio para sesión WhatsApp: ${sessionPath}`
      );
    }
    ...
    if (!fs.existsSync(workerScript)) {
      console.error(
        `[Server][CRITICAL] No se encuentra el script del worker: ${workerScript}. No se puede iniciar el worker para ${userId}.`
      );
      try {
        ...
        console.error(
          "[Server][Firestore Error] Error actualizando estado a error (worker script missing):",
          dbErr
        );
      }
      return null;
    ...
    if (userDocSnap.exists) {
      ...
      console.warn(
        `[Server] Documento de usuario ${userId} no encontrado en Firestore al iniciar worker.`
      );
      // Considerar si crear el documento aquí o dejar que falle?
      // Por ahora, continuaremos, el worker usará default.
    }
    console.log(
      `[Server] Agente activo inicial para ${userId}: ${
        activeAgentId || "Ninguno (usará default)"
      }`
    );
    ...
    console.log(
      `[Server][DEBUG] Worker PID ${worker.pid} added to workers map for ${userId}.`
    ); // Log adding to map
    ...
    console.log(
      `[Server][DB Firestore] Usuario ${userId} worker_pid -> ${worker.pid}, status -> connecting`
    );
    // <<< END MODIFICATION >>>
    ...
    worker.on("exit", async (code, signal) => {
      console.log(
        `[Server] Worker para ${userId} (PID: ${
          worker.pid || "N/A"
        }) terminó inesperadamente con código ${code}, señal ${signal}`
      );
      ...
      try {
        ...
        if (
          workerExisted &&
          (!statusDocSnap.exists ||
            statusDocSnap.data().status !== "disconnected")
        ) {
          const exitErrorMsg = `Worker exited code ${code}${
          ...
          console.log(
            `[Server][DB Firestore] Usuario ${userId} worker_pid -> null, status -> error (Worker exit)`
          );
        } else {
          console.log(
            `[Server] No se actualiza subcolección status en exit para ${userId}, estado ya era disconnected o worker no registrado.`
          );
        }
        // <<< ADDED: Release lock on exit >>>
        connectingUsers.delete(userId);
        console.log(
          `[Server][DEBUG] Releasing lock (worker exit) for ${userId}. connectingUsers:`,
          connectingUsers
        );
        // <<< END ADDED >>>
        // <<< END MODIFICATION >>>
        ...
        console.error(
          "[Server][Firestore Error] Error obteniendo/actualizando status en exit:",
          dbErr
        );
      }
    });
    ...
    worker.on("error", async (error) => {
      console.error(
        `[Server] Error en worker ${userId} (PID: ${worker.pid || "N/A"}):`,
        error
      );
      ...
      connectingUsers.delete(userId);
      console.log(
        `[Server][DEBUG] Releasing lock (worker error event) for ${userId}. connectingUsers:`,
        connectingUsers
      );
      // <<< END ADDED >>>
      try {
        ...
        console.log(
          `[Server][DB Firestore] Usuario ${userId} worker_pid -> null, status -> error (Worker error event)`
        );
        // <<< END MODIFICATION >>>
      } catch (dbErr) {
        console.error(
          "[Server][Firestore Error] Error actualizando DB en error de worker:",
          dbErr
        );
      }
    });
    ...
    connectingUsers.delete(userId);
    console.log(
      `[Server][DEBUG] Releasing lock (after successful start sequence) for ${userId}. connectingUsers:`,
      connectingUsers
    );
    // <<< END ADDED >>>
    ...
    console.error(
      `[Server][CRITICAL] Error CRÍTICO iniciando worker para ${userId}:`,
      error
    );
    // <<< ADDED: Ensure lock is released on critical error >>>
    connectingUsers.delete(userId);
    console.log(
      `[Server][DEBUG] Releasing lock (critical start error) for ${userId}. connectingUsers:`,
      connectingUsers
    );
    // <<< END ADDED >>>
    ...
    try {
      ...
      const statusDocRefOnError = userDocRefOnError
        .collection("status")
        .doc("whatsapp");
      ...
      console.error(
        `[Server][Firestore Error] Error secundario actualizando estado a error crítico para ${userId}:`,
        dbErr
      );
    }
    return null;
...
async function fetchInitialConfigsAndNotifyWorker(userId, activeAgentId) {
  console.log(
    `[Server] Preparando configuración inicial para worker ${userId} (Agente: ${
  ...
  try {
    ...
    if (activeAgentId) {
      ...
      if (agentDocSnap.exists) {
        ...
        console.log(
          `   -> Configuración encontrada para agente ${activeAgentId}`
        );
      } else {
        console.warn(
          `   -> Agente ${activeAgentId} especificado pero no encontrado en Firestore. Worker usará default.`
        );
      }
    } else {
      console.log(
        `   -> No hay agente activo especificado. Worker usará default.`
      );
    }
    ...
    // 3. Obtener Starters
    const startersSnapshot = await userDocRef
      .collection("gemini_starters")
      .get();
    ...
    notifyWorker(userId, {
      type: "INITIAL_CONFIG",
      payload: initialConfigPayload,
    });
    console.log(
      `[Server] Configuración inicial enviada a worker ${userId} via IPC.`
    );
  } catch (error) {
    console.error(
      `[Server][Firestore Error] Error crítico obteniendo configuración inicial para ${userId}:`,
      error
    );
...
async function stopWorker(userId) {
  ...
  if (workers[userId] && workers[userId].connected) {
    console.log(
      `[Server] Iniciando parada para worker ${userId} (PID: ${workers[userId].pid})`
    );
    try {
      ...
      console.log(
        `[Server][DB Firestore] Usuario ${userId} worker_pid -> null, status -> disconnected (manual stop)`
      );
      // <<< END MODIFICATION >>>
      ...
      if (workers[userId] && workers[userId].connected) {
        ...
        console.warn(
          `[Server] Worker ${userId} ya no está conectado al intentar enviar SHUTDOWN.`
        );
      ...
      console.error(
        "[Server][Firestore Error] Error actualizando DB antes de parar worker:",
        dbErr
      );
      ...
      try {
        if (workers[userId] && workers[userId].connected) {
          console.log(
            `[Server] Enviando comando SHUTDOWN a worker ${userId} (a pesar de error DB)`
          );
          workers[userId].send({ type: "COMMAND", command: "SHUTDOWN" });
        }
      } catch (ipcErr) {
        console.error(
          `[Server] Error enviando SHUTDOWN (tras error DB) a worker ${userId}:`,
          ipcErr
        );
      }
    ...
    console.log(
      `[Server] Worker para ${userId} no encontrado o no conectado. Asegurando estado Firestore.`
    );
    try {
      ...
      console.log(
        `[Server][DB Firestore] Asegurado ${userId} worker_pid -> null, status -> disconnected (worker not found)`
      );
      // <<< END MODIFICATION >>>
    } catch (dbErr) {
      console.error(
        "[Server][Firestore Error] Error asegurando desconexión en Firestore (worker no encontrado):",
        dbErr
      );
    }
...
async function handleWorkerMessage(userId, message) {
  console.log(
    `[Server][CRITICAL] handleWorkerMessage de ${userId}, tipo: ${
      message?.type || "DESCONOCIDO"
    }, mensaje completo:`,
    JSON.stringify(message)
  );
  ...
  if (!message || !message.type) {
    console.error(
      `[Server][ERROR] Mensaje IPC inválido recibido de worker ${userId}`
    );
    return;
  }
  ...
  if (message.type === "NEW_MESSAGE_RECEIVED") {
    console.log(
      `[Server][IPC] Received NEW_MESSAGE_RECEIVED from worker ${userId}`
    );
    ...
    if (clientWs && clientWs.readyState === WebSocket.OPEN) {
      try {
        ...
        clientWs.send(
          JSON.stringify({ type: "newMessage", data: message.payload })
        );
        console.log(
          `[Server][WebSocket] Sent newMessage notification to user ${userId}`
        );
      } catch (wsError) {
        console.error(
          `[Server][WebSocket] Error sending message to user ${userId}:`,
          wsError
        );
      }
    } else {
      ...
      if (!clientWs) {
        console.log(
          `[Server][WebSocket] No active client found in wsClients map for user ${userId}. Cannot send newMessage notification.`
        );
      } else {
        console.log(
          `[Server][WebSocket] Client found for user ${userId} but not open (readyState: ${clientWs.readyState}). Cannot send newMessage notification.`
        );
      }
    }
  ...
  switch (message.type) {
    ...
    default:
      console.log(
        `[Server] Mensaje tipo ${message.type} no manejado para worker ${userId}.`
      );
      return;
  }
  ...
  try {
    ...
    const statusDocRef = firestoreDb
      .collection("users")
      .doc(userId)
      .collection("status")
      .doc("whatsapp");
    ...
    console.error(
      `[Server][IPC Master] Error actualizando Firestore (status/whatsapp) para worker ${userId} por mensaje ${message.type}:`,
      err
    );
  }
}
...
app.post("/users", async (req, res) => {
  ...
  console.log(
    `[Server] POST /users - Intentando registrar usuario: ${trimmedUserId}`
  );
  ...
  try {
    ...
    console.error(
      "[Server][Firestore Error] Error creando/verificando usuario:",
      err
    );
    return res
      .status(500)
...
app.get("/users", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error interno al obtener usuarios." });
  }
});
// --- Fin Reemplazo Firestore ---
...
app.post("/users/:userId/connect", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      userData.status === "connecting" ||
      userData.status === "generating_qr"
    ) {
      if (workers[userId] && workers[userId].connected) {
        console.log(
    ...
    console.error(
      "[Server][Firestore Error] Error verificando usuario para conectar:",
      err
    );
    return res
      .status(500)
      .json({ success: false, message: "Error interno al verificar usuario." });
  }
});
// --- Fin Reemplazo Firestore ---
...
app.post("/users/:userId/disconnect", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "[Server][Firestore Error] Error verificando usuario para desconectar:",
      err
    );
...
app.get("/users/:userId/status", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "[Server][Firestore Error] Error obteniendo estado de usuario:",
      err
    );
    return res
      .status(500)
      .json({ success: false, message: "Error interno al obtener estado." });
  }
});
// --- Fin Reemplazo Firestore ---
...
app.get("/users/:userId/active-agent", async (req, res) => {
  ...
  try {
    ...
    if (!activeAgentId) {
      return res.status(404).json({ // Usar 404 es correcto si el recurso (agente activo) no existe
            success: false, 
        message: "No active agent set for user." 
        });
    }

    res.json({
      success: true,
      message: "Agente activo recuperado.",
      data: {
    ...
    console.error(`[Server][Firestore Error] Error obteniendo agente activo para ${userId}:`, err);
    res.status(500).json({
      success: false,
      message: "Error interno al obtener el agente activo.",
    });
  }
});
// --- FIN NUEVA RUTA ---
...
app.post("/users/:userId/send-message", async (req, res) => {
  ...
  if (!number || !message || !number.trim() || !message.trim()) {
    return res
      .status(400)
      .json({ success: false, message: "Número y mensaje son requeridos." });
  }
  ...
  try {
    ...
    if (statusDocSnap.exists) {
      ...
      console.log(
        `[Server][Send Message Check] Firestore status for ${userId}: ${currentStatus}`
      );
    } else {
      // If status doc doesn't exist, assume not connected
      console.warn(
        `[Server][Send Message Check] Firestore status document not found for ${userId}. Assuming not connected.`
      );
    ...
    if (!workers[userId] || !workers[userId].connected) {
      console.error(
        `[Server][Send Message Check] Error: Firestore status is 'connected' for ${userId}, but worker process is missing or IPC disconnected!`
      );
    ...
    console.log(
      `[Server][Send Message Check] Firestore status 'connected' and worker process found for ${userId}. Proceeding with IPC.`
    );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error interno verificando estado." });
  }
  // --- END MODIFICATION ---
...
app.get("/users/:userId/rules", async (req, res) => {
  ...
  try {
    const rulesSnapshot = await firestoreDb
      .collection("users")
      .doc(userId)
      .collection("rules")
      .get();
    ...
    console.error(
      `[Server][Firestore Error] Error listando reglas para ${userId}:`,
      err
    );
    if (err.code === 5 || (err.message && err.message.includes("NOT_FOUND"))) {
      // 5 = gRPC NOT_FOUND
      console.warn(
        `[Server] Usuario ${userId} no encontrado o sin colección de reglas.`
      );
      return res.json({ success: true, data: [] });
    }
    res
      .status(500)
      .json({ success: false, message: "Error interno al listar reglas." });
  }
});
...
app.post("/users/:userId/add-rule", async (req, res) => {
  ...
  if (!trigger || !response || !trigger.trim() || !response.trim()) {
    return res
      .status(400)
      .json({ success: false, message: "Trigger y response son requeridos." });
  }
  const trimmedTrigger = trigger.trim().toLowerCase();
  const trimmedResponse = response.trim();
  const rulesCollectionRef = firestoreDb
    .collection("users")
    .doc(userId)
    .collection("rules");
  ...
  try {
    ...
    const duplicateQuery = await rulesCollectionRef
      .where("trigger", "==", trimmedTrigger)
      .limit(1)
      .get();
    ...
    notifyWorker(userId, {
      ...
      payload: { rules: updatedRulesData },
    });
    res
      .status(201)
      .json({ success: true, message: "Regla añadida.", data: newRule });
  } catch (err) {
    console.error(
      `[Server][Firestore Error] Error añadiendo regla para ${userId}:`,
      err
    );
    res
      .status(500)
      .json({ success: false, message: "Error interno al guardar la regla." });
  }
});
...
app.delete("/users/:userId/rules/:ruleId", async (req, res) => {
  ...
  try {
    ...
    if (!docSnap.exists) {
      return res
        .status(404)
        .json({ success: false, message: "Regla no encontrada." });
    }

    await ruleDocRef.delete();
    // <<< UPDATED: Notificar al worker para que recargue las reglas (CON PAYLOAD) >>>
    const remainingRulesSnapshot = await firestoreDb
      .collection("users")
      .doc(userId)
      .collection("rules")
      .get(); // Re-fetch remaining
    const remainingRulesData = remainingRulesSnapshot.docs.map((doc) =>
      doc.data()
    );
    notifyWorker(userId, {
      type: "COMMAND",
      command: "RELOAD_RULES",
      payload: { rules: remainingRulesData },
    });
    res.json({ success: true, message: "Regla eliminada." });
  } catch (err) {
    console.error(
      `[Server][Firestore Error] Error eliminando regla ${ruleId} para ${userId}:`,
      err
    );
    res
      .status(500)
      .json({ success: false, message: "Error interno al eliminar la regla." });
  }
});
...
app.get("/users/:userId/gemini-starters", async (req, res) => {
  ...
  try {
    const startersSnapshot = await firestoreDb
      .collection("users")
      .doc(userId)
      .collection("gemini_starters")
      .get();
    ...
    console.error(
      `[Server][Firestore Error] Error listando starters para ${userId}:`,
      err
    );
    if (err.code === 5 || (err.message && err.message.includes("NOT_FOUND"))) {
      // 5 = gRPC NOT_FOUND
      console.warn(
        `[Server] Usuario ${userId} no encontrado o sin colección de gemini_starters.`
      );
      return res.json({ success: true, data: [] });
    }
    res.status(500).json({
...
app.post("/users/:userId/add-gemini-starter", async (req, res) => {
  ...
  if (!trigger || !prompt || !trigger.trim() || !prompt.trim()) {
    return res
      .status(400)
      .json({ success: false, message: "Trigger y prompt son requeridos." });
  }
  const trimmedTrigger = trigger.trim().toLowerCase();
  const trimmedPrompt = prompt.trim();
  const startersCollectionRef = firestoreDb
    .collection("users")
    .doc(userId)
    .collection("gemini_starters");
  ...
  try {
    ...
    // Verificar duplicados por trigger
    const duplicateQuery = await startersCollectionRef
      .where("trigger", "==", trimmedTrigger)
      .limit(1)
      .get();
    if (!duplicateQuery.empty) {
    ...
    const updatedStartersData = updatedStartersSnapshot.docs.map((doc) =>
      doc.data()
    );
    notifyWorker(userId, {
      type: "COMMAND",
      command: "RELOAD_STARTERS",
...
app.delete("/users/:userId/gemini-starters/:starterId", async (req, res) => {
  ...
  const starterDocRef = firestoreDb
    .collection("users")
    .doc(userId)
    .collection("gemini_starters")
    .doc(starterId);
  ...
  try {
    ...
    if (!docSnap.exists) {
      return res
        .status(404)
        .json({ success: false, message: "Disparador no encontrado." });
    }

    await starterDocRef.delete();
    // <<< UPDATED: Notificar al worker para que recargue los starters (CON PAYLOAD) >>>
    const remainingStartersSnapshot = await firestoreDb
      .collection("users")
      .doc(userId)
      .collection("gemini_starters")
      .get(); // Re-fetch remaining
    const remainingStartersData = remainingStartersSnapshot.docs.map((doc) =>
      doc.data()
    );
    notifyWorker(userId, {
      type: "COMMAND",
      command: "RELOAD_STARTERS",
      payload: { starters: remainingStartersData },
    });
...
app.get("/users/:userId/action-flows", async (req, res) => {
  // <-- CAMBIO DE RUTA
  ...
  try {
    ...
    const flowsSnapshot = await firestoreDb
      .collection("users")
      .doc(userId)
      .collection("action_flows")
      .orderBy("createdAt", "desc")
      .get(); // <-- CAMBIO DE RUTA FIRESTORE
    const flows = [];
    ...
    console.error(
      `[Server][Firestore Error] Error listando flujos de acción para ${userId}:`,
      err
    ); // <-- ACTUALIZAR LOG
    if (err.code === 5 || (err.message && err.message.includes("NOT_FOUND"))) {
      // Puede ser error al encontrar user o collection
      console.warn(
        `[Server] Usuario ${userId} no encontrado o sin colección de action_flows.`
      );
      return res.json({ success: true, data: [] }); // Devolver vacío si no existe user o colección
    }
    res
      .status(500)
      .json({ success: false, message: "Error interno al listar flujos." });
  }
});

// POST /users/:userId/action-flows - Crear un nuevo flujo para un usuario
app.post("/users/:userId/action-flows", async (req, res) => {
  // <-- CAMBIO DE RUTA
  const userId = req.params.userId; // <-- OBTENER userId
  ...
  const flowsCollectionRef = firestoreDb
    .collection("users")
    .doc(userId)
    .collection("action_flows"); // <-- RUTA FIRESTORE USER-SPECIFIC
  const userDocRef = firestoreDb.collection("users").doc(userId);
  ...
  try {
    ...
    console.log(
      `[Server] Flujo de acción creado para ${userId} en Firestore: ${flowId} - ${newFlow.name}`
    );

    // Notificar al worker específico de este usuario
    console.log(
      `[Server] Recargando y notificando flujos al worker ${userId}...`
    );
    ...
    notifyWorker(userId, {
      type: "RELOAD_USER_FLOWS",
      payload: { flows: userFlowsData },
    }); // <-- NUEVO COMANDO Y PAYLOAD
...
app.get("/users/:userId/action-flows/:flowId", async (req, res) => {
  ...
  try {
    ...
    if (!flowDoc.exists) {
      return res
        .status(404)
        .json({ success: false, message: "Flujo de acción no encontrado." });
    }
    // Convertir Timestamps si es necesario para el cliente
    ...
    console.error(
      `[Server][Firestore Error] Error obteniendo flujo ${flowId} para ${userId}:`,
      err
    ); // <-- ACTUALIZAR LOG
    res
      .status(500)
      .json({ success: false, message: "Error interno al obtener el flujo." });
  }
});

// PUT /users/:userId/action-flows/:flowId - Actualizar un flujo existente de un usuario
app.put("/users/:userId/action-flows/:flowId", async (req, res) => {
  // <-- CAMBIO DE RUTA
  const { userId, flowId } = req.params; // <-- OBTENER userId Y flowId
  ...
  if (
    !updatedFlowData ||
    typeof updatedFlowData !== "object" ||
    !updatedFlowData.name ||
  ...
  const flowDocRef = firestoreDb
    ...
    .doc(userId)
    .collection("action_flows")
    .doc(flowId); // <-- CAMBIO DE RUTA FIRESTORE
  const flowsCollectionRef = firestoreDb
    .collection("users")
    .doc(userId)
    .collection("action_flows"); // Para recargar
  ...
  try {
    ...
    console.log(
      `[Server] Flujo de acción actualizado para ${userId} en Firestore: ${flowId} - ${dataToUpdate.name}`
    );
    ...
    const userFlowsDataUpdate = userFlowsSnapshotUpdate.docs.map((doc) =>
      doc.data()
    );
    ...
    notifyWorker(userId, {
      type: "RELOAD_USER_FLOWS",
      payload: { flows: userFlowsDataUpdate },
    }); // <-- NUEVO COMANDO Y PAYLOAD

    // Merge local aproximado para respuesta
    const approxUpdatedData = {
      ...updatedFlowData,
      id: flowId,
      updatedAt: new Date().toISOString(),
    };
    res.json({
      success: true,
      message: "Flujo de acción actualizado.",
      data: approxUpdatedData,
    });
  } catch (err) {
    console.error(
      `[Server][Firestore Error] Error actualizando flujo ${flowId} para ${userId}:`,
      err
    ); // <-- ACTUALIZAR LOG
    if (err.code === 5) {
      // Firestore NOT_FOUND
...
app.delete("/users/:userId/action-flows/:flowId", async (req, res) => {
  // <-- CAMBIO DE RUTA
  const { userId, flowId } = req.params; // <-- OBTENER userId Y flowId
  ...
  try {
    ...
    console.log(
      `[Server] Flujo de acción eliminado de Firestore: ${flowId} para usuario ${userId}`
    );
    ...
    const userFlowsDataDelete = userFlowsSnapshotDelete.docs.map((doc) =>
      doc.data()
    );
    ...
    notifyWorker(userId, {
      type: "RELOAD_USER_FLOWS",
      payload: { flows: userFlowsDataDelete },
    }); // <-- NUEVO COMANDO Y PAYLOAD

    res.json({ success: true, message: "Flujo de acción eliminado." });
  } catch (err) {
    console.error(
      `[Server][Firestore Error] Error eliminando flujo ${flowId} para ${userId}:`,
      err
    ); // <-- ACTUALIZAR LOG
    res
      .status(500)
      .json({ success: false, message: "Error interno al eliminar el flujo." });
  }
});
...
app.get("/users/:userId/agents", async (req, res) => {
  ...
  try {
    const agentsSnapshot = await firestoreDb
      .collection("users")
      .doc(userId)
      .collection("agents")
      .get();
    ...
    console.error(
      `[Server][Firestore Error] Error listando agentes para ${userId}:`,
      err
    );
    if (err.code === 5 || (err.message && err.message.includes("NOT_FOUND"))) {
      console.warn(
        `[Server] Usuario ${userId} no encontrado o sin colección de agentes.`
      );
      return res.json({ success: true, data: [] }); // Devolver vacío si no existe user o colección
    }
    res
      .status(500)
      .json({ success: false, message: "Error interno al listar agentes." });
  }
});
...
app.post("/users/:userId/agents", async (req, res) => {
  ...
  const agentsCollectionRef = firestoreDb
    .collection("users")
    .doc(userId)
    .collection("agents");
  ...
  try {
    ...
    console.log(
      `[Server] Agente creado para ${userId}: ${agentId} - ${newAgent.persona.name}`
    );
    ...
    res
      .status(500)
      .json({ success: false, message: "Error interno al guardar el agente." });
  }
});
...
app.get("/users/:userId/agents/:agentId", async (req, res) => {
  ...
  try {
    ...
    if (!docSnap.exists) {
      return res
        .status(404)
        .json({ success: false, message: "Agente no encontrado." });
    }
    res.json({ success: true, data: docSnap.data() });
  } catch (err) {
    console.error(
      `[Server][Firestore Error] Error obteniendo agente ${agentId} para ${userId}:`,
      err
    );
    res
      .status(500)
      .json({ success: false, message: "Error interno al obtener el agente." });
  }
});
...
app.put("/users/:userId/agents/:agentId", async (req, res) => {
  ...
  if (
    !updatedAgentData ||
    typeof updatedAgentData !== "object" ||
    !updatedAgentData.persona ||
  ...
  // Validar que knowledge sea un objeto (puede venir vacío)
  if (
    typeof updatedAgentData.knowledge !== "object" ||
    updatedAgentData.knowledge === null
  ) {
  ...
  try {
    ...
    console.log(
      `[Server] Agente actualizado para ${userId}: ${agentId} - ${dataToUpdate.persona.name}`
    );
    ...
    if (userDoc.exists && userDoc.data().active_agent_id === agentId) {
      console.log(
        ` -> Notificando worker ${userId} por cambio en agente activo.`
      );
      ...
      console.log(
        ` -> Agente actualizado no era el activo para ${userId}, no se notifica cambio inmediato.`
      );
    }
    ...
    res.json({
      ...
      data: { ...dataToUpdate, id: agentId },
    });
  } catch (err) {
    console.error(
      `[Server][Firestore Error] Error actualizando agente ${agentId} para ${userId}:`,
      err
    );
    if (err.code === 5) {
      // Firestore NOT_FOUND
...
app.delete("/users/:userId/agents/:agentId", async (req, res) => {
  ...
  try {
    ...
    if (userDocSnap.exists && userDocSnap.data().active_agent_id === agentId) {
      console.log(
        ` -> Desasignando agente activo ${agentId} de usuario ${userId} antes de eliminar.`
      );
      await userDocRef.update({
    ...
    console.log(
      `[Server] Agente eliminado de Firestore: ${agentId} para usuario ${userId}`
    );

    // Notificar al worker si se eliminó el agente activo
    if (activeAgentWasDeleted) {
      console.log(
        ` -> Notificando worker ${userId} que su agente activo fue eliminado.`
      );
...
app.put("/users/:userId/active-agent", async (req, res) => {
  ...
  // <<< ADDED LOG >>>
  console.log(
    `[Server][SwitchAgent] PUT /users/${userId}/active-agent - Intentando establecer a: ${agentId}`
  );
  ...
  try {
    ...
    console.log(
      `[Server][SwitchAgent] Agente activo para ${userId} actualizado en Firestore a ${
        agentId || "ninguno"
      }.`
    );
    ...
    // <<< ADDED LOG >>>
    console.log(
      `[Server][SwitchAgent] Notificando worker ${userId} para cambiar al agente ${
        agentId || "default"
      }. Payload config: ${
        agentConfigPayload
          ? JSON.stringify(agentConfigPayload).substring(0, 200) + "..."
          : "null"
      }`
    ); // Log truncado
    notifyWorker(userId, {
    ...
    res.json({
      success: true,
      message: `Agente activo establecido a ${agentId || "ninguno"}.`,
...
app.get("/users/:userId/chats", async (req, res) => {
  ...
  try {
    ...
    const chatsSnapshot = await firestoreDb
      .collection("users")
      .doc(userId)
      .collection("chats")
    ...
    console.error(
      `[Server][Firestore Error] Error fetching chats for user ${userId}:`,
      err
    );
...
app.get("/users/:userId/chats/:chatId/messages", async (req, res) => {
  ...
  console.log(
    `[Server] GET /users/${userId}/chats/${chatId}/messages (limit: ${limit}, before: ${beforeTimestampStr})`
  );
  ...
  try {
    ...
    const messages = messagesSnapshot.docs.map((doc) => {
      ...
      return {
        ...
        timestamp: data.timestamp?.toDate
          ...
          : data.timestamp, // Convert Firestore Timestamp
        to: data.to,
        type: data.type,
        vCards: data.vCards || [],
    ...
    console.log(
      `[Server] GET /users/${userId}/chats/${chatId}/messages - Returning ${
        messages.length
      } messages (limited to ${limit}). First message 'fromMe': ${
        messages.length > 0 ? messages[0].fromMe : "N/A"
      }`
    ); // <<< UPDATED LOG >>>
    ...
    console.error(
      `[Server][Firestore Error] Error fetching messages for chat ${chatId}, user ${userId}:`,
      err
    );
    // Handle specific errors like missing indices
    if (err.code === 5) {
      // NOT_FOUND (likely chat doesn't exist)
      return res
        .status(404)
...
// <<< NEW: API Endpoint to Send Message to a Chat >>>
app.post("/users/:userId/chats/:chatId/messages", async (req, res) => {
  const { userId, chatId } = req.params;
  const { message, origin } = req.body;
  
  console.log(`[Server] POST /users/${userId}/chats/${chatId}/messages - Message: "${message?.substring(0, 30)}..." Origin: ${origin}`);
  ...
  try {
    // Check Firestore status to ensure user is connected
    const statusDocRef = firestoreDb
      .collection("users")
      .doc(userId)
      .collection("status")
    ...
    if (statusDocSnap.exists) {
      currentStatus = statusDocSnap.data().status;
      isConnected = currentStatus === "connected";
      console.log(`[Server][Send Message] Firestore status for ${userId}: ${currentStatus}`);
    } else {
      console.warn(`[Server][Send Message] Firestore status document not found for ${userId}. Assuming not connected.`);
      currentStatus = "not_found";
    }

    // Check if the status is 'connected'
    if (!isConnected) {
      console.warn(`[Server] Attempt to send message for ${userId} but Firestore status is NOT 'connected' (is: ${currentStatus}).`);
      return res.status(400).json({
        success: false,
        message: `Usuario ${userId} no está conectado a WhatsApp (estado: ${currentStatus}). Conéctese primero.`,
      });
    }

    // Check if the worker process exists in memory (needed for IPC)
    if (!workers[userId] || !workers[userId].connected) {
      console.error(`[Server][Send Message] Error: Firestore status is 'connected' for ${userId}, but worker process is missing or IPC disconnected!`);
      return res.status(500).json({
        success: false,
        message: "Error interno: Inconsistencia entre estado y proceso worker.",
      });
    ...
    console.log(`[Server][Send Message] Firestore status 'connected' and worker process found for ${userId}. Proceeding with IPC.`);

    // Send command to worker via IPC
    workers[userId].send({
      type: "COMMAND",
      command: "SEND_MESSAGE",
      payload: { 
        number: chatId, // Use chatId as the phone number/chat identifier
        message: message.trim() 
      },
    });

    console.log(`[Server] ✅ Message send command sent to worker for ${userId} -> ${chatId}`);
    ...
    console.error(`[Server][Firestore Error] Error sending message for chat ${chatId}, user ${userId}:`, err);
    res.status(500).json({
      success: false,
...
// <<< END: API Endpoint to Send Message to a Chat >>>
...
app.put("/users/:userId/chats/:chatId/contact-name", async (req, res) => {
  ...
  const { name } = req.body;
  console.log(
    `[Server] PUT /users/${userId}/chats/${chatId}/contact-name - Setting name to: ${name}`
  );
  ...
  try {
    ...
    console.log(
      `[Server] Contact display name for chat ${chatId} updated to "${name.trim()}".`
    );
    res.json({
      success: true,
      message: "Nombre del contacto actualizado con éxito.",
...
try {
  ...
  server.listen(port, () => {
    console.log(
      `¡Server API Principal (v2) escuchando en http://localhost:${port}! (WebSocket ready)`
    );
  });
...
process.on("SIGINT", () => {
  ...
  setTimeout(() => {
    ...
    server.close(() => {
      // <<< CHANGED: Close the http server instance
      console.log("[Server] Servidor HTTP cerrado.");

      // Intentar detener todos los workers activos
      const activeWorkerPIDs = Object.values(workers)
        .map((w) => w?.pid)
        .filter(Boolean);
      console.log(
        `[Server] Intentando detener ${
          Object.keys(workers).length
        } workers activos...`
      );
      const stopPromises = Object.keys(workers).map((userId) =>
        stopWorker(userId)
      ); // stopWorker now returns a promise implicitly
        ...
        setTimeout(() => {
          ...
          Object.values(workers).forEach((worker) => {
            if (worker && !worker.killed) {
              console.warn(
                `[Server] Forzando terminación del worker PID: ${worker.pid}`
              );
              worker.kill("SIGTERM");
            }
          });
...
try {
  ...
  if (!geminiApiKey) {
    console.warn(
      "[Server][Gemini] GEMINI_API_KEY environment variable is not defined. Prompt helper will not work."
    );
  } else {
    const genAI = new GoogleGenerativeAI(geminiApiKey);
    serverGeminiModel = genAI.getGenerativeModel({ model: "gemini-1.5-flash" }); // Or your preferred model
    console.log(
      "[Server][Gemini] Gemini Model initialized successfully for server utilities."
    );
  }
} catch (geminiInitError) {
  console.error(
    "[Server][Gemini] ERROR Initializing GoogleGenerativeAI for server utilities:",
    geminiInitError
  );
}
...
app.post(
  "/users/:userId/generate-assisted-prompt",
  authenticateApiKey,
  async (req, res) => {
    const userId = req.params.userId;
    ...
    console.log(
      `[Server] POST /users/${userId}/generate-assisted-prompt - Received data:`,
      req.body
    );
      ...
      followupResponses.forEach((item, index) => {
        if (item.question && (item.answer || item.selectedOptions)) {
          ...
          if (responseValue && responseValue.trim()) {
            followupSection += `    ${String.fromCharCode(97 + index)}. **${
              item.question
            }**\n`;
...
6.  **¿Necesita acceso a herramientas/funciones específicas?** ${
      needsTools ? "Sí" : "No"
    }
...
11. **Principal Llamada a la Acción que el agente debe impulsar:**
    ${
      primaryCallToAction ||
      "Asistir al usuario y resolver su consulta de la mejor manera posible"
    }${followupSection}
...
"""
Eres ${agentNameOrRole || "un asistente virtual"}, operando para ${
      companyOrContext || "nuestra empresa"
    }. Tu principal objetivo es ${objective}.
Tu tono debe ser consistentemente ${desiredTone || "profesional y amigable"}.

Al interactuar con los clientes, que principalmente son ${
...
${
  keyInfoToInclude
    ? `- Asegúrate de mencionar o utilizar la siguiente información clave: ${keyInfoToInclude}`
    : "- No hay información clave específica predefinida para mencionar."
}

Comportamiento a Evitar:
${
  thingsToAvoid
    ? `- Absolutamente evita: ${thingsToAvoid}`
    : "- No hay comportamientos específicos a evitar predefinidos."
}

Flujo de Conversación y Herramientas:
Si el cliente pregunta por "${
      expectedInputs || "información general"
    }", tu respuesta debería enfocarse en "${
      expectedOutputs || "proporcionar asistencia general"
    }".
${
  needsTools && tools
    ? `Para ello, puedes hacer uso de las siguientes herramientas: ${tools}. Explica su utilidad si es relevante.`
    : ""
}

Tu objetivo final es guiar al cliente hacia: ${
      primaryCallToAction || "la resolución de su consulta"
    }.
...
Si alguna información no fue proporcionada (ej. "Herramientas Específicas" si "Necesita herramientas" es no), omite esa sección o adáptala inteligentemente.`;
    ...
    try {
      console.log(
        `[Server][Gemini] Enviando meta-prompt mejorado para ${userId}. Longitud: ${metaPrompt.length}`
      );
      ...
      console.log(
        `[Server][Gemini] Prompt (mejorado) generado para ${userId} (longitud: ${generatedPrompt.length}):`,
        generatedPrompt.substring(0, 200) + "..."
      );
      res.json({ success: true, generatedPrompt: generatedPrompt.trim() });
    } catch (error) {
      console.error(
        `[Server][Gemini] Error generando prompt mejorado para ${userId}:`,
        error
      );
      ...
      if (
        ...
        // More generic check for safety messages
        return res.status(400).json({
          success: false,
          message:
            "La IA no pudo generar el prompt debido a restricciones de contenido. Intenta reformular tus entradas.",
        });
      }
      // Generic error
      res.status(500).json({
...
app.post(
  "/users/:userId/generate-followup-questions",
  authenticateApiKey,
  async (req, res) => {
    const userId = req.params.userId;
    ...
    console.log(
      `[Server] POST /users/${userId}/generate-followup-questions - Received data:`,
      req.body
    );
...
- Para selecciones únicas, usa type: "radio" o "select" con opciones pertinentes`;
    ...
    try {
      console.log(
        `[Server][Gemini] Generando preguntas de seguimiento para ${userId}`
      );
      ...
      try {
        ...
        if (
          ...
          !Array.isArray(questionsObject.questions)
        ) {
          throw new Error("Formato JSON inválido: falta array 'questions'");
        }

        console.log(
          `[Server][Gemini] Generadas ${questionsObject.questions.length} preguntas de seguimiento para ${userId}`
        );
        res.json({
          success: true,
        ...
        console.error(
          `[Server][Gemini] Error parseando JSON de preguntas para ${userId}:`,
          jsonError
        );
        ...
        return res.status(500).json({
          success: false,
          message:
            "Error procesando las preguntas generadas. El formato no es válido.",
          raw: generatedText.substring(0, 1000), // Devolver texto crudo para diagnóstico
        });
      }
    } catch (error) {
      console.error(
        `[Server][Gemini] Error generando preguntas de seguimiento para ${userId}:`,
        error
      );
...
app.post("/users/:userId/kanban-boards", async (req, res) => {
  ...
  const { name } = req.body;
  console.log(
    `[Server] POST /users/${userId}/kanban-boards - Creando tablero: ${name}`
  );

  if (!name || !name.trim()) {
    return res
      .status(400)
      .json({ success: false, message: "El nombre del tablero es requerido." });
  }
...
app.get("/users/:userId/kanban-boards", async (req, res) => {
  ...
  try {
    ...
    const boardsSnapshot = await userDocRef
...
app.get("/users/:userId/kanban-boards/:boardId", async (req, res) => {
  ...
  try {
    ...
    if (!docSnap.exists) {
      return res
        .status(404)
        .json({ success: false, message: "Tablero Kanban no encontrado." });
    }
    const data = docSnap.data();
    if (data.createdAt?.toDate)
      data.createdAt = data.createdAt.toDate().toISOString();
...
app.put("/users/:userId/kanban-boards/:boardId", async (req, res) => {
  ...
  try {
    ...
    if (columns_order && Array.isArray(columns_order)) {
    ...
    console.log(
      `[Server] Tablero Kanban actualizado para ${userId}: ${boardId}`
    );
    const updatedDocSnap = await boardDocRef.get();
    res.json({
      success: true,
      message: "Tablero Kanban actualizado.",
      data: updatedDocSnap.data(),
    });
  } catch (err) {
    console.error(
      `[Server][Firestore Error] Actualizando tablero ${boardId} para ${userId}:`,
      err
    );
    if (err.code === 5) {
      // Firestore NOT_FOUND
...
app.delete("/users/:userId/kanban-boards/:boardId", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[Server] Tablero Kanban eliminado de Firestore: ${boardId} para ${userId}`
    );
...
app.post("/users/:userId/kanban-boards/:boardId/columns", async (req, res) => {
  ...
  console.log(
    `[Server] POST /users/${userId}/kanban-boards/${boardId}/columns - Creando columna: ${name}, Tipo Etapa: ${
      stageType || "No definido"
    }`
  );
...
app.get("/users/:userId/kanban-boards/:boardId/columns", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `[Server][Firestore Error] Listando columnas para tablero ${boardId}:`,
      err
    );
    res
      .status(500)
      .json({ success: false, message: "Error interno al listar columnas." });
  }
});

// PUT /users/:userId/kanban-boards/:boardId/columns/:columnId - Actualizar una columna
app.put(
  "/users/:userId/kanban-boards/:boardId/columns/:columnId",
  async (req, res) => {
    const { userId, boardId, columnId } = req.params;
    const { name, stageType } = req.body; // Added stageType
    console.log(
      `[Server] PUT /users/${userId}/kanban-boards/${boardId}/columns/${columnId} - Datos: ${JSON.stringify(
        req.body
      )}`
    );
    ...
    try {
      ...
      res.json({
        success: true,
        message: "Columna Kanban actualizada.",
        data: updatedDocSnap.data(),
      });
    } catch (err) {
      console.error(
        `[Server][Firestore Error] Actualizando columna ${columnId}:`,
        err
      );
      if (err.code === 5) {
        // Firestore NOT_FOUND
...
app.delete(
  ...
  async (req, res) => {
    const { userId, boardId, columnId } = req.params;
    console.log(
      `[Server] DELETE /users/${userId}/kanban-boards/${boardId}/columns/${columnId}`
    );
    ...
    try {
      ...
      if (!columnDocSnap.exists) {
        return res
          .status(404)
          .json({ success: false, message: "Columna Kanban no encontrada." });
      }
...
app.put(
  "/users/:userId/chats/:chatId/assign-kanban-column",
  async (req, res) => {
    const { userId, chatId } = req.params;
    ...
    console.log(
      `[Server] PUT /users/${userId}/chats/${chatId}/assign-kanban-column - Board: ${boardId}, Column: ${columnId}, ContactName: ${contactName}`
    );
    ...
    try {
      ...
      console.log(
        `[Server] Chat ${chatId} asignado a tablero ${
          boardId || "ninguno"
        }, columna ${
          columnId || "ninguna"
        }. Nombre de contacto actualizado si se proporcionó.`
      );
      res.json({
        success: true,
        message: `Chat asignado a columna ${columnId || "ninguna"} en tablero ${
          boardId || "ninguno"
        }.`,
      });
    } catch (err) {
      console.error(
        `[Server][Firestore Error] Asignando chat ${chatId} a Kanban:`,
        err
      );
      if (err.code === 5) {
        return res.status(404).json({
...
app.get(
  "/users/:userId/kanban-boards/:boardId/chats-by-column",
  async (req, res) => {
    const { userId, boardId } = req.params;
    console.log(
      `[Server] GET /users/${userId}/kanban-boards/${boardId}/chats-by-column`
    );
    ...
    try {
      ...
      columnsSnapshot.forEach((doc) => {
        ...
        if (colData.createdAt?.toDate)
          colData.createdAt = colData.createdAt.toDate().toISOString();
        if (colData.updatedAt?.toDate)
          colData.updatedAt = colData.updatedAt.toDate().toISOString();
      ...
      chatsInBoardSnapshot.forEach((doc) => {
        ...
        if (chatData.lastMessageTimestamp?.toDate)
          chatData.lastMessageTimestamp = chatData.lastMessageTimestamp
            .toDate()
            .toISOString();
        if (chatData.createdAt?.toDate)
          chatData.createdAt = chatData.createdAt.toDate().toISOString();
        if (chatData.updatedAt?.toDate)
          chatData.updatedAt = chatData.updatedAt.toDate().toISOString();

        // <<< ADDED: Include contactDisplayName in chatData for Kanban card >>>
        chatData.contactDisplayName = chatData.contactDisplayName || null;
        
        // DEBUG: Log contact display name for debugging
        console.log(`[Server] Chat ${doc.id} contactDisplayName: "${chatData.contactDisplayName}" (original: "${doc.data().contactDisplayName}")`);
      ...
      if (responseBoardData.createdAt?.toDate)
        responseBoardData.createdAt = responseBoardData.createdAt
          .toDate()
          .toISOString();
      if (responseBoardData.updatedAt?.toDate)
        responseBoardData.updatedAt = responseBoardData.updatedAt
          .toDate()
          .toISOString();
      ...
      console.error(
        `[Server][Firestore Error] Obteniendo chats por columna para tablero ${boardId}:`,
        err
      );
      if (
        err.message &&
...
// <<< ADDED: Endpoint for Instagram bot to get Gemini replies >>>
app.post("/api/instagram/generate-reply", async (req, res) => {
  console.log("[DEBUG] /api/instagram/generate-reply called");
  console.log("[DEBUG] Headers:", req.headers);
  console.log("[DEBUG] Body:", req.body);
  const { message, history, instagramUserId, threadId, configUserId, agentId } =
    req.body;

  // Validaciones básicas
  if (!message || !instagramUserId || !threadId) {
    console.error(
      "[Server Instagram] Missing required fields: message, instagramUserId, or threadId."
    );
    return res.status(400).json({
      success: false,
  ...
  if (!configUserId || !agentId) {
    console.error(
      "[Server Instagram] Missing configuration fields: configUserId or agentId."
    );
    return res.status(400).json({
      success: false,
      error: "Missing configuration fields: configUserId or agentId",
    });
  }

  let agentBasePrompt = ""; // Default a vacío
  ...
  try {
    ...
    console.log(
      `[Server Instagram] Attempting to load starters from: ${startersPath}`
    );

    if (fs.existsSync(startersPath)) {
      const startersFileContent = fs.readFileSync(startersPath, "utf-8");
      const starters = JSON.parse(startersFileContent);

      const selectedAgent = starters.find((agent) => agent.trigger === agentId);

      if (selectedAgent && selectedAgent.prompt) {
        agentBasePrompt = selectedAgent.prompt;
        console.log(
          `[Server Instagram] Agent prompt for config '${configUserId}' (agentId '${agentId}') loaded successfully. Length: ${agentBasePrompt.length}`
        );
      } else {
        console.warn(
          `[Server Instagram] Agent with trigger '${agentId}' not found or has no prompt in ${startersPath} for config '${configUserId}'. Using default/no base prompt.`
        );
      }
    } else {
      console.warn(
        `[Server Instagram] starters.json not found at ${startersPath} for config '${configUserId}'. Using default/no base prompt.`
      );
    }
  } catch (err) {
    console.error(
      `[Server Instagram] Error loading or parsing agent starters for config '${configUserId}':`,
      err
    );
    // No se devuelve error al cliente aquí, se procederá sin prompt de agente específico si falla la carga.
  }

  try {
    // La historia de new.py ya debería estar en el formato correcto [{role: "user", parts: [...]}, {role: "model", parts: [...]}]
    const conversationContents = await buildPromptWithHistoryForInstagram(
      threadId, // Usado para logging/contexto en buildPrompt
      message,
      agentBasePrompt, // Este ahora podría no usarse directamente en buildPrompt si se pasa a callGemini como systemInstruction
      history || [] // Asegurar que el historial sea un array
    );

    const geminiReply = await callGeminiWithRetryInstagram(
      threadId, // Usado para logging/contexto en callGemini
      conversationContents,
      agentBasePrompt // Se pasa aquí para que callGemini lo use como systemInstruction
    );
    ...
    if (geminiReply) {
      ...
      console.error(
        `[Server Instagram] Failed to get reply from Gemini for threadId: ${threadId}. callGeminiWithRetryInstagram returned empty.`
      );
      res
        .status(500)
        .json({ success: false, error: "Failed to get reply from Gemini" });
    }
  } catch (error) {
    console.error(
      `[Server Instagram] Error processing Instagram message with Gemini for threadId ${threadId}:`,
      error
    );
    res.status(500).json({
      success: false,
      error: "Internal server error processing Gemini request",
    });
  }
});
...
function buildPromptWithHistoryForInstagram(
  threadId,
  message,
  agentBasePrompt,
  history
) {
  // Stub simple: combina el historial y el mensaje actual en un array
  // Puedes mejorar la lógica según tus necesidades
  const prompt = [];
  if (agentBasePrompt) {
    prompt.push({ role: "system", content: agentBasePrompt });
  }
...
async function callGeminiWithRetryInstagram(
  threadId,
  conversationContents,
  agentBasePrompt
) {
  if (!geminiModelInstance) {
    console.error("[Gemini][DEBUG] geminiModelInstance is not initialized");
  ...
  if (Array.isArray(conversationContents)) {
    for (const item of conversationContents) {
      if (item.role === "user") {
        prompt += `Usuario: ${
          item.content || (item.parts && item.parts[0]?.text) || ""
        }\n`;
        ...
        prompt += `Sistema: ${item.content}\n`;
      }
    }
  } else if (typeof conversationContents === "string") {
    prompt += conversationContents;
  }

  console.log("[Gemini][DEBUG] Prompt enviado a Gemini:", prompt);

  try {
    const result = await geminiModelInstance.generateContent(prompt);
    console.log("[Gemini][DEBUG] Respuesta cruda de Gemini:", result);
    ...
    console.log("[Gemini][DEBUG] Texto extraído de Gemini:", text);
    if (text && text.length > MAX_REPLY_LENGTH) {
      return text.slice(0, MAX_REPLY_LENGTH) + "…";
    }
    return text || "No se pudo obtener respuesta de Gemini.";
  } catch (err) {
    console.error("[Gemini][DEBUG] Error al generar respuesta:", err);
    if (err && err.response) {
      console.error("[Gemini][DEBUG] Error response:", err.response);
    }
    throw err;
  }
}

// ==== ENDPOINTS PARA GESTIONAR DISPARADORES INICIALES DE CONVERSACIÓN ====

// Obtener todos los disparadores iniciales de un usuario
app.get("/users/:userId/initial-triggers", async (req, res) => {
  const { userId } = req.params;
  console.log(`[Server] GET /users/${userId}/initial-triggers`);

  try {
    const triggersSnapshot = await firestoreDb.collection('users').doc(userId).collection('initial_triggers').get();
    ...
    console.error(`[Server] Error obteniendo disparadores iniciales:`, error);
    return res.status(500).json({ 
      success: false, 
      message: `Error obteniendo disparadores: ${error.message}` 
    });
  }
});

// Añadir un nuevo disparador inicial
app.post("/users/:userId/initial-triggers", async (req, res) => {
  const { userId } = req.params;
  const { text, type = 'contains', description = '' } = req.body;
  console.log(`[Server] POST /users/${userId}/initial-triggers`, req.body);

  if (!text || text.trim() === '') {
    return res.status(400).json({
      success: false,
      message: "El texto del disparador es requerido"
    });
  }

  try {
    // Validar tipo de disparador
    const validTypes = ['exact', 'contains', 'starts_with'];
    const triggerType = validTypes.includes(type) ? type : 'contains';
    ...
    const triggersRef = firestoreDb.collection('users').doc(userId).collection('initial_triggers');
    const newTriggerDoc = await triggersRef.add(triggerData);

    // Notificar al worker
    if (workers[userId]) {
      notifyWorker(userId, {
        type: "RELOAD_INITIAL_TRIGGERS"
      });
    }
    ...
    return res.json({
      ...
      message: "Disparador inicial creado exitosamente"
    });
  } catch (error) {
    console.error(`[Server] Error creando disparador inicial:`, error);
    return res.status(500).json({ 
      success: false, 
      message: `Error creando disparador: ${error.message}` 
    });
  }
});

// Eliminar un disparador inicial
app.delete("/users/:userId/initial-triggers/:triggerId", async (req, res) => {
  const { userId, triggerId } = req.params;
  console.log(`[Server] DELETE /users/${userId}/initial-triggers/${triggerId}`);

  try {
    // Verificar si existe el disparador
    const triggerRef = firestoreDb.collection('users').doc(userId).collection('initial_triggers').doc(triggerId);
    const triggerDoc = await triggerRef.get();

    if (!triggerDoc.exists) {
      return res.status(404).json({
        success: false,
        message: "Disparador inicial no encontrado"
      });
    }
    ...
    return res.json({
      success: true,
      message: "Disparador inicial eliminado exitosamente"
    });
  } catch (error) {
    console.error(`[Server] Error eliminando disparador inicial:`, error);
    return res.status(500).json({ 
      success: false, 
      message: `Error eliminando disparador: ${error.message}` 
    });
  }
});

// Endpoint para reiniciar/desactivar todos los chats
app.post("/users/:userId/reset-chat-activations", async (req, res) => {
  const { userId } = req.params;
  console.log(`[Server] POST /users/${userId}/reset-chat-activations`);

  try {
    // Obtener todos los chats del usuario
    const chatsSnapshot = await firestoreDb.collection('users').doc(userId).collection('chats').get();
    
    if (chatsSnapshot.empty) {
      return res.json({ 
        success: true, 
        message: "No hay chats para reiniciar",
        count: 0
      });
    }

    // Array para operaciones batch
    const batches = [];
    let currentBatch = firestoreDb.batch();
    let operationCount = 0;
    let batchCount = 0;

    // Procesar cada documento en lotes (máximo 500 operaciones por lote)
    chatsSnapshot.forEach(doc => {
      const chatRef = firestoreDb.collection('users').doc(userId).collection('chats').doc(doc.id);
      currentBatch.update(chatRef, { isActivated: false });
      operationCount++;

      // Si alcanzamos el límite, crear un nuevo lote
    ...
    // Añadir el último lote si tiene operaciones
    if (operationCount > 0) {
      batches.push(currentBatch);
      batchCount++;
    }
    ...
    if (workers[userId]) {
      notifyWorker(userId, {
        type: "CHATS_DEACTIVATED"
      });
    }

    return res.json({
      success: true,
      message: `Se reiniciaron ${chatsSnapshot.size} chats exitosamente`,
      count: chatsSnapshot.size,
      batches: batchCount
    });
  } catch (error) {
    console.error(`[Server] Error reiniciando activación de chats:`, error);
    return res.status(500).json({ 
      success: false, 
      message: `Error reiniciando chats: ${error.message}` 
    });
  }
});

// POST /users/:userId/assign-chat-to-kanban - Asignar chat a columna de Kanban
app.post("/users/:userId/assign-chat-to-kanban", async (req, res) => {
  const { userId } = req.params;
  const { chatId, boardId, columnId, contactDisplayName } = req.body;
  
  console.log(`[Server] POST /users/${userId}/assign-chat-to-kanban - Chat: ${chatId}, Board: ${boardId}, Column: ${columnId}, ContactName: ${contactDisplayName}`);

  if (!chatId) {
    return res.status(400).json({
      success: false,
      message: "El campo 'chatId' es requerido."
    });
  }
  ...
  try {
    ...
    // Si se proporciona contactDisplayName, incluirlo en la actualización
    if (contactDisplayName !== undefined && typeof contactDisplayName === 'string') {
      updateData.contactDisplayName = contactDisplayName.trim();
      console.log(`[Server] Updating contact display name to: "${contactDisplayName.trim()}"`);
    } else {
      console.log(`[Server] No contactDisplayName provided or invalid type. Value: ${contactDisplayName}, Type: ${typeof contactDisplayName}`);
    }

    // DEBUG: Log completo de updateData antes de la actualización
    console.log(`[Server] Complete updateData object:`, JSON.stringify(updateData, null, 2));

    // Si se proporciona boardId y columnId, asignar a esa columna
    if (boardId && columnId) {
      ...
      if (!boardDocSnap.exists) {
        return res.status(404).json({
          success: false,
          message: "Tablero Kanban no encontrado."
        });
      }
      ...
      if (!columnDocSnap.exists) {
        return res.status(404).json({
          success: false,
          message: "Columna Kanban no encontrada."
        });
      }

      updateData.kanbanBoardId = boardId;
      updateData.kanbanColumnId = columnId;
      
      console.log(`[Server] Assigning chat ${chatId} to board ${boardId}, column ${columnId}`);
    } 
    // Si solo se proporciona boardId (sin columnId), asignar al tablero pero sin columna específica
    else if (boardId && !columnId) {
      // Verificar que el tablero existe
      const boardDocRef = firestoreDb
        .collection("users")
      ...
      updateData.kanbanBoardId = boardId;
      updateData.kanbanColumnId = admin.firestore.FieldValue.delete(); // Remover columnId
      
      console.log(`[Server] Assigning chat ${chatId} to board ${boardId} (no specific column)`);
    }
    // Si no se proporciona boardId ni columnId, desasignar del Kanban completamente
    else {
      updateData.kanbanBoardId = admin.firestore.FieldValue.delete();
      updateData.kanbanColumnId = admin.firestore.FieldValue.delete();
      
      console.log(`[Server] Unassigning chat ${chatId} from Kanban`);
    }

    // DEBUG: Log final de updateData antes de enviar a Firestore
    console.log(`[Server] Final updateData before Firestore update:`, JSON.stringify(updateData, null, 2));

    // Actualizar el chat
    await chatDocRef.update(updateData);

    console.log(`[Server] ✅ Chat ${chatId} Kanban assignment updated successfully`);
    
    // DEBUG: Verificar que el dato se guardó correctamente
    const updatedChatSnap = await chatDocRef.get();
    const updatedChatData = updatedChatSnap.data();
    console.log(`[Server] 🔍 Verification - Chat data after update:`, {
      chatId: chatId,
      contactDisplayName: updatedChatData.contactDisplayName,
      kanbanBoardId: updatedChatData.kanbanBoardId,
      kanbanColumnId: updatedChatData.kanbanColumnId
    });
    ...
    res.status(200).json({
      ...
      data: {
        ...
        contactDisplayName: contactDisplayName || null,
        timestamp: new Date().toISOString()
      }
    });

  } catch (err) {
    console.error(`[Server][Firestore Error] Error assigning chat ${chatId} to Kanban:`, err);
    res.status(500).json({
      success: false,
      message: "Error interno al asignar chat a Kanban.",
    });
  }
});

// ==== ENDPOINTS PARA GESTIONAR DISPARADORES INICIALES DE CONVERSACIÓN ====
// REFACTOR: These endpoints now manage triggers PER AGENT, not per user.

// Obtener todos los disparadores de un agente
app.get("/users/:userId/agents/:agentId/triggers", async (req, res) => {
  const { userId, agentId } = req.params;
  console.log(`[Server] GET /users/${userId}/agents/${agentId}/triggers`);
  
  try {
    const triggersSnapshot = await firestoreDb.collection('users').doc(userId).collection('agents').doc(agentId).collection('triggers').get();
    ...
    console.error(`[Server] Error obteniendo disparadores para el agente ${agentId}:`, error);
    return res.status(500).json({
      success: false,
      message: `Error obteniendo disparadores: ${error.message}`
    });
...
// Añadir un nuevo disparador a un agente
app.post("/users/:userId/agents/:agentId/triggers", async (req, res) => {
  const { userId, agentId } = req.params;
  const { text, type } = req.body;
  
  console.log(`[Server] POST /users/${userId}/agents/${agentId}/triggers`, req.body);
  
  if (!text || !text.trim()) {
    return res.status(400).json({ success: false, message: "El texto del disparador es requerido" });
  }
  ...
  try {
    ...
    const triggersRef = firestoreDb.collection('users').doc(userId).collection('agents').doc(agentId).collection('triggers');
    const newTriggerDoc = await triggersRef.add(triggerData);

    // Notificar al worker para que recargue los disparadores (si es necesario)
    notifyWorker(userId, { type: "RELOAD_INITIAL_TRIGGERS", payload: { agentId } });
    ...
    return res.status(201).json({
      ...
      message: "Disparador creado exitosamente"
    });

  } catch (error) {
    console.error(`[Server] Error creando disparador para el agente ${agentId}:`, error);
    return res.status(500).json({
      success: false,
      message: `Error creando disparador: ${error.message}`
    });
...
// Eliminar un disparador de un agente
app.delete("/users/:userId/agents/:agentId/triggers/:triggerId", async (req, res) => {
  const { userId, agentId, triggerId } = req.params;
  console.log(`[Server] DELETE /users/${userId}/agents/${agentId}/triggers/${triggerId}`);
  
  try {
    const triggerRef = firestoreDb.collection('users').doc(userId).collection('agents').doc(agentId).collection('triggers').doc(triggerId);
    const triggerDoc = await triggerRef.get();
    ...
    // Notificar al worker para que recargue los disparadores
    notifyWorker(userId, { type: "RELOAD_INITIAL_TRIGGERS", payload: { agentId } });

    return res.json({
      success: true,
      message: "Disparador eliminado exitosamente"
    });

  } catch (error) {
    console.error(`[Server] Error eliminando disparador para el agente ${agentId}:`, error);
    return res.status(500).json({
      success: false,
      message: `Error eliminando disparador: ${error.message}`
    });
...
// Eliminar un disparador inicial - ESTA RUTA DEBERÍA QUEDAR OBSOLETA Y ELIMINARSE A FUTURO
app.delete("/users/:userId/initial-triggers/:triggerId", async (req, res) => {
  // ... (código sin cambios, se mantiene por retrocompatibilidad si fuera necesario)
});

// Actualizar un disparador existente de un agente
app.put("/users/:userId/agents/:agentId/triggers/:triggerId", async (req, res) => {
  const { userId, agentId, triggerId } = req.params;
  const { text, type } = req.body;
  
  console.log(`[Server] PUT /users/${userId}/agents/${agentId}/triggers/${triggerId}`, req.body);
  ...
  try {
    ...
    // Verificar que el disparador exista antes de actualizar
    ...
    await triggerRef.update(triggerData);
    ...
    return res.json({
      ...
      message: "Disparador actualizado exitosamente"
    });

  } catch (error) {
    console.error(`[Server] Error actualizando disparador ${triggerId} para el agente ${agentId}:`, error);
    return res.status(500).json({
      success: false,
      message: `Error actualizando disparador: ${error.message}`
    });
  }
});

// === FUNCIÓN NUCLEAR DE LIMPIEZA COMPLETA ===

/**
 * FUNCIÓN NUCLEAR: Elimina COMPLETAMENTE todos los datos de un usuario
 * @param {string} userId - ID del usuario a eliminar
 * @returns {Promise<Object>} - Resultado de la limpieza
 */
async function nukeUserDataCompletely(userId) {
    console.log(`[NUKE] ====== INICIANDO LIMPIEZA NUCLEAR PARA ${userId} ======`);
    ...
    try {
        // ===== PASO 1: TERMINAR PROCESOS ACTIVOS =====
        console.log(`[NUKE] PASO 1: Terminando procesos activos...`);
        await terminateActiveProcesses(userId, cleanupResults);
        
        // ===== PASO 2: LIMPIAR MEMORIA =====
        console.log(`[NUKE] PASO 2: Limpiando memoria...`);
        await clearMemoryData(userId, cleanupResults);
        
        // ===== PASO 3: LIMPIAR WEBSOCKETS =====
        console.log(`[NUKE] PASO 3: Cerrando WebSockets...`);
        await closeWebSocketConnections(userId, cleanupResults);
        
        // ===== PASO 4: DESTRUIR SESIÓN WHATSAPP =====
        console.log(`[NUKE] PASO 4: Destruyendo sesión WhatsApp...`);
        await destroyWhatsAppSession(userId, cleanupResults);
        
        // ===== PASO 5: LIMPIAR ARCHIVOS DEL SISTEMA =====
        console.log(`[NUKE] PASO 5: Eliminando archivos del sistema...`);
        await deleteFileSystemData(userId, cleanupResults);
        
        // ===== PASO 6: LIMPIAR FIREBASE/FIRESTORE =====
        console.log(`[NUKE] PASO 6: Eliminando datos de Firestore...`);
        await deleteFirestoreData(userId, cleanupResults);
        
        // ===== PASO 7: LIMPIAR LOCALSTORAGE (INSTRUCCIONES) =====
        console.log(`[NUKE] PASO 7: Generando instrucciones para localStorage...`);
        await generateLocalStorageCleanupInstructions(userId, cleanupResults);
        
        // ===== PASO 8: VERIFICACIÓN FINAL =====
        console.log(`[NUKE] PASO 8: Verificando limpieza...`);
        await verifyCleanupComplete(userId, cleanupResults);
        
        cleanupResults.success = true;
        console.log(`[NUKE] ====== LIMPIEZA NUCLEAR COMPLETADA PARA ${userId} ======`);
        
    } catch (error) {
        console.error(`[NUKE] ERROR CRÍTICO en limpieza nuclear:`, error);
...
// ===== PASO 1: TERMINAR PROCESOS ACTIVOS =====
async function terminateActiveProcesses(userId, results) {
    const step = { name: 'terminate_processes', items: [], errors: [] };
    
    try {
        // 1.1 Enviar comando DESTROY_SESSION al worker
        if (workers[userId] && workers[userId].connected) {
            try {
                workers[userId].send({ type: "COMMAND", command: "DESTROY_SESSION" });
                await new Promise(resolve => setTimeout(resolve, 3000)); // Esperar 3s
                step.items.push('Worker DESTROY_SESSION command sent');
            } catch (ipcError) {
                step.errors.push(`IPC DESTROY_SESSION failed: ${ipcError.message}`);
            }
        }
        
        // 1.2 Forzar terminación del worker
        if (workers[userId]) {
            try {
                workers[userId].kill('SIGTERM');
                delete workers[userId];
                step.items.push('Worker process terminated');
            } catch (killError) {
                step.errors.push(`Worker kill failed: ${killError.message}`);
            }
        }
        
        // 1.3 Limpiar set de usuarios conectándose
        if (connectingUsers.has(userId)) {
            connectingUsers.delete(userId);
            step.items.push('Removed from connectingUsers set');
        }
        
    } catch (error) {
        step.errors.push(`Process termination error: ${error.message}`);
    }
...
// ===== PASO 2: LIMPIAR MEMORIA =====
async function clearMemoryData(userId, results) {
    const step = { name: 'clear_memory', items: [], errors: [] };
    
    try {
        // 2.1 Limpiar variables globales del worker (si existe)
        // Esto se hace enviando un comando especial o matando el proceso
        step.items.push('Worker memory cleared via process termination');
        
        // 2.2 Limpiar tracking de tokens Gemini
        // Se limpia automáticamente al terminar el worker
        step.items.push('Gemini token tracking cleared');
        
        // 2.3 Limpiar cache de configuración
        // Se limpia automáticamente al terminar el worker
        step.items.push('Configuration cache cleared');
        
    } catch (error) {
        step.errors.push(`Memory clear error: ${error.message}`);
    }
...
// ===== PASO 3: LIMPIAR WEBSOCKETS =====
async function closeWebSocketConnections(userId, results) {
    const step = { name: 'close_websockets', items: [], errors: [] };
    
    try {
        if (wsClients.has(userId)) {
            const ws = wsClients.get(userId);
            if (ws.readyState === WebSocket.OPEN) {
                ws.close(1001, 'User data cleanup');
                step.items.push('WebSocket connection closed');
            }
            wsClients.delete(userId);
            step.items.push('WebSocket client removed from map');
        }
        
    } catch (error) {
        step.errors.push(`WebSocket close error: ${error.message}`);
    }
...
// ===== PASO 4: DESTRUIR SESIÓN WHATSAPP =====
async function destroyWhatsAppSession(userId, results) {
    const step = { name: 'destroy_whatsapp_session', items: [], errors: [] };
    
    try {
        // 4.1 Usar el endpoint existente de destrucción
        // Esto ya se hizo en el paso 1, pero verificamos
        step.items.push('WhatsApp session destruction initiated');
        
        // 4.2 Esperar a que se complete la destrucción
        await new Promise(resolve => setTimeout(resolve, 2000));
        step.items.push('WhatsApp session destruction completed');
        
    } catch (error) {
        step.errors.push(`WhatsApp session destroy error: ${error.message}`);
    }
...
// ===== PASO 5: LIMPIAR ARCHIVOS DEL SISTEMA =====
async function deleteFileSystemData(userId, results) {
    const step = { name: 'delete_filesystem', items: [], errors: [] };
    
    try {
        const userDataDir = path.join(__dirname, "data_v2", userId);
        const sessionPath = path.join(userDataDir, ".wwebjs_auth");
        const uploadsPath = path.join(userDataDir, "uploads");
        
        // 5.1 Eliminar archivos de sesión WhatsApp
        if (fs.existsSync(sessionPath)) {
            fs.rmSync(sessionPath, { recursive: true, force: true });
            step.items.push(`WhatsApp session files deleted: ${sessionPath}`);
        }
        
        // 5.2 Eliminar archivos de uploads
        if (fs.existsSync(uploadsPath)) {
            fs.rmSync(uploadsPath, { recursive: true, force: true });
            step.items.push(`Upload files deleted: ${uploadsPath}`);
        }
        
        // 5.3 Eliminar directorio completo del usuario
        if (fs.existsSync(userDataDir)) {
            fs.rmSync(userDataDir, { recursive: true, force: true });
            step.items.push(`User data directory deleted: ${userDataDir}`);
        }
        
        // 5.4 Limpiar archivos temporales de Puppeteer
        try {
            const tempDirs = fs.readdirSync('/tmp').filter(dir => 
                dir.includes('puppeteer') && dir.includes(userId)
            );
            
            tempDirs.forEach(tempDir => {
                const tempPath = path.join('/tmp', tempDir);
                if (fs.existsSync(tempPath)) {
                    fs.rmSync(tempPath, { recursive: true, force: true });
                    step.items.push(`Temp directory deleted: ${tempPath}`);
                }
            });
        } catch (tempError) {
            step.errors.push(`Temp directory cleanup error: ${tempError.message}`);
        }
        
    } catch (error) {
        step.errors.push(`Filesystem delete error: ${error.message}`);
    }
...
// ===== PASO 6: LIMPIAR FIREBASE/FIRESTORE =====
async function deleteFirestoreData(userId, results) {
    const step = { name: 'delete_firestore', items: [], errors: [] };
    
    try {
        const userDocRef = firestoreDb.collection('users').doc(userId);
        
        // 6.1 Eliminar todas las subcollecciones
        const subcollections = [
            'agents', 'rules', 'gemini_starters', 'action_flows', 
            'campaigns', 'chats', 'status', 'blacklists', 'whitelists',
            'kanban_boards', 'requestLogs', 'templates', 'initial_triggers'
        ];
        
        for (const subcollection of subcollections) {
            try {
                await deleteCollection(userDocRef.collection(subcollection));
                step.items.push(`Subcollection deleted: ${subcollection}`);
            } catch (subError) {
                step.errors.push(`Error deleting ${subcollection}: ${subError.message}`);
            }
        }
        
        // 6.2 Eliminar documento principal del usuario
        await userDocRef.delete();
        step.items.push('Main user document deleted');
        
        // 6.3 Eliminar referencias en otras colecciones (si existen)
        // Por ejemplo, si hay referencias cruzadas en otras partes de la DB
        step.items.push('Cross-references cleaned');
        
    } catch (error) {
        step.errors.push(`Firestore delete error: ${error.message}`);
    }
    
    results.steps.push(step);
}

// ===== PASO 7: GENERAR INSTRUCCIONES PARA LOCALSTORAGE =====
async function generateLocalStorageCleanupInstructions(userId, results) {
    const step = { name: 'localstorage_instructions', items: [], errors: [] };
    ...
    try {
        const localStorageKeys = [
            ...
            'instagram_device_id',
            'instagram_cookies',
            'instagram_2fa_session',
            `whatsapp_status_${userId}`,
...
${localStorageKeys.map(key => `localStorage.removeItem('${key}');`).join('\n')}
sessionStorage.clear();
// Limpiar cookies específicas si es necesario
...
console.log('LocalStorage limpiado para usuario ${userId}');
        ...
        step.errors.push(`LocalStorage instructions error: ${error.message}`);
    }
...
// ===== PASO 8: VERIFICACIÓN FINAL =====
async function verifyCleanupComplete(userId, results) {
    const step = { name: 'verify_cleanup', items: [], errors: [] };
    
    try {
        // 8.1 Verificar que no hay worker activo
        if (!workers[userId]) {
            step.items.push('✅ No active worker process');
        } else {
            step.errors.push('❌ Worker process still exists');
        }
        
        // 8.2 Verificar que no hay WebSocket
        if (!wsClients.has(userId)) {
            step.items.push('✅ No active WebSocket connection');
        } else {
            step.errors.push('❌ WebSocket connection still exists');
        }
        
        // 8.3 Verificar archivos del sistema
        const userDataDir = path.join(__dirname, "data_v2", userId);
        if (!fs.existsSync(userDataDir)) {
            step.items.push('✅ User data directory removed');
        } else {
            step.errors.push('❌ User data directory still exists');
        }
        
        // 8.4 Verificar Firestore
        const userDocRef = firestoreDb.collection('users').doc(userId);
        const userDoc = await userDocRef.get();
        if (!userDoc.exists) {
            step.items.push('✅ User document removed from Firestore');
        } else {
            step.errors.push('❌ User document still exists in Firestore');
        }
        
        // 8.5 Verificar set de usuarios conectándose
        if (!connectingUsers.has(userId)) {
            step.items.push('✅ Removed from connecting users set');
        } else {
            step.errors.push('❌ Still in connecting users set');
        }
        
    } catch (error) {
        step.errors.push(`Verification error: ${error.message}`);
    }
    
    results.steps.push(step);
}

// ===== FUNCIÓN AUXILIAR: ELIMINAR COLECCIÓN COMPLETA =====
...
/**
 * CHECKLIST: Verificar que la limpieza fue exitosa
 * @param {string} userId - ID del usuario
 * @returns {Promise<Object>} - Resultado de la verificación
 */
async function verifyUserDataCleanup(userId) {
    const verification = {
        userId: userId,
        timestamp: new Date().toISOString(),
        allClean: true,
    ...
    // ✅ CHECK 1: Procesos activos
    verification.checks.push({
        name: 'Active Processes',
        clean: !workers[userId],
        details: workers[userId] ? 'Worker still exists' : 'No worker process'
    });
    
    // ✅ CHECK 2: WebSocket connections
    verification.checks.push({
        name: 'WebSocket Connections',
        clean: !wsClients.has(userId),
        details: wsClients.has(userId) ? 'WebSocket still active' : 'No WebSocket connection'
    });
    
    // ✅ CHECK 3: Connecting users set
    verification.checks.push({
        name: 'Connecting Users Set',
        clean: !connectingUsers.has(userId),
        details: connectingUsers.has(userId) ? 'Still in connecting set' : 'Not in connecting set'
    });
    
    // ✅ CHECK 4: File system
    const userDataDir = path.join(__dirname, "data_v2", userId);
    const filesExist = fs.existsSync(userDataDir);
    verification.checks.push({
        name: 'File System',
        clean: !filesExist,
        details: filesExist ? `Files still exist at ${userDataDir}` : 'No files found'
    });
    ...
    try {
        ...
        verification.checks.push({
            name: 'Firestore',
            clean: !userDoc.exists,
            details: userDoc.exists ? 'User document still exists' : 'No user document found'
        });
    } catch (error) {
        verification.checks.push({
            name: 'Firestore',
            clean: false,
            details: `Error checking Firestore: ${error.message}`
        });
    }
    ...
    try {
        ...
        verification.checks.push({
            ...
            clean: tempDirs.length === 0,
            details: tempDirs.length > 0 ? `Found ${tempDirs.length} temp directories` : 'No temp files found'
        });
    } catch (error) {
        verification.checks.push({
            name: 'Temporary Files',
            clean: false,
            details: `Error checking temp files: ${error.message}`
        });
    }
    
    // Determinar si todo está limpio
    verification.allClean = verification.checks.every(check => check.clean);
    
    return verification;
}

// === ENDPOINTS DE LIMPIEZA NUCLEAR ===

// ENDPOINT: GET /cleanup/status - ESTADO DEL SISTEMA
app.get("/cleanup/status", authenticateApiKey, async (req, res) => {
    try {
        const activeUsers = Object.keys(workers).length;
        const activeConnections = wss.clients.size;
        const firestoreCollections = ['users', 'agents', 'rules', 'gemini_starters', 'action_flows', 'chats', 'messages'];
        ...
        res.json({
            ...
            system: {
                ...
                firestoreCollections: firestoreCollections.length,
...
// ENDPOINT: POST /nuke-all-users - LIMPIEZA NUCLEAR DE TODOS LOS USUARIOS
app.post("/nuke-all-users", authenticateApiKey, async (req, res) => {
    const { confirmationCode } = req.body;
    
    if (confirmationCode !== "NUKE_ALL_CONFIRMED") {
        return res.status(400).json({
            success: false,
            message: "Código de confirmación inválido para limpieza masiva",
            instructions: "Para confirmar la eliminación masiva, envía el código: NUKE_ALL_CONFIRMED"
        });
    }
    
    console.log('[NUKE ALL] Iniciando limpieza nuclear de TODOS los usuarios');
    
    try {
        const allUsers = Object.keys(workers);
        const results = {
        ...
        for (const userId of allUsers) {
            try {
                console.log(`[NUKE ALL] Limpiando usuario: ${userId}`);
                const cleanupResult = await nukeUserDataCompletely(userId);
                ...
                results.failed++;
                results.errors.push(`Usuario ${userId}: ${error.message}`);
                results.details.push({
                    userId: userId,
                    status: 'error',
                    error: error.message
        ...
        if (results.success) {
            res.json({
                success: true,
                message: `Limpieza nuclear completada para ${results.successful} usuarios`,
                results: results
            });
        } else {
            res.status(207).json({
                success: false,
                message: `Limpieza completada con errores. ${results.successful} exitosos, ${results.failed} fallidos`,
                results: results
            });
        }
        
    } catch (error) {
        console.error('[NUKE ALL] Error crítico:', error);
        res.status(500).json({
            success: false,
            message: "Error crítico en limpieza masiva",
            error: error.message
        });
    }
});

// ENDPOINT: POST /users/:userId/nuke - LIMPIEZA NUCLEAR COMPLETA
app.post("/users/:userId/nuke", authenticateApiKey, async (req, res) => {
    const userId = req.params.userId;
    const { confirmationCode } = req.body;
    ...
    if (confirmationCode !== expectedCode) {
        return res.status(400).json({
            ...
            instructions: `Para confirmar la eliminación, envía el código: ${expectedCode}`
        });
    }
    
    console.log(`[NUKE API] Iniciando limpieza nuclear para usuario: ${userId}`);
    
    try {
        const cleanupResults = await nukeUserDataCompletely(userId);
        ...
        if (cleanupResults.success) {
            ...
            res.status(500).json({
                success: false,
                message: "Limpieza nuclear completada con errores",
                results: cleanupResults
            });
        }
        
    } catch (error) {
        console.error(`[NUKE API] Error en limpieza nuclear:`, error);
        res.status(500).json({
            success: false,
            message: "Error crítico en limpieza nuclear",
            error: error.message
        });
    }
});

// ENDPOINT: GET /users/:userId/cleanup/verify - VERIFICAR LIMPIEZA
app.get("/users/:userId/cleanup/verify", async (req, res) => {
    const userId = req.params.userId;
    
    try {
        const verification = await verifyUserDataCleanup(userId);
        ...
        console.error(`[VERIFY API] Error verificando limpieza:`, error);
        res.status(500).json({
            success: false,
...
// ENDPOINT: POST /nuke-all-users - LIMPIEZA NUCLEAR DE TODOS LOS USUARIOS
app.post("/nuke-all-users", async (req, res) => {
    const { confirmationCode } = req.body;
    
    // Código de confirmación especial para limpiar TODOS los usuarios
    const expectedCode = `NUKE_ALL_${Date.now().toString().slice(-6)}`;
    ...
    if (confirmationCode !== expectedCode) {
        return res.status(400).json({
            ...
            message: "Código de confirmación inválido para limpieza masiva",
            expectedCode: expectedCode,
            warning: "⚠️ ESTO ELIMINARÁ TODOS LOS USUARIOS Y SUS DATOS PERMANENTEMENTE"
        });
    }
    
    console.log(`[NUKE ALL API] Iniciando limpieza nuclear MASIVA para TODOS los usuarios`);
    ...
    try {
        ...
        console.log(`[NUKE ALL] Encontrados ${allUsers.length} usuarios para limpiar`);
        ...
        for (const userId of allUsers) {
            try {
                ...
                console.error(`[NUKE ALL] Error limpiando usuario ${userId}:`, userError);
                results.userResults.push({
                    userId: userId,
                    success: false,
        ...
        res.json({
            success: true,
            message: `Limpieza masiva completada. ${results.successfulCleanups} exitosas, ${results.failedCleanups} fallidas`,
            results: results
        });
        
    } catch (error) {
        console.error(`[NUKE ALL API] Error en limpieza masiva:`, error);
        res.status(500).json({
            success: false,
            message: "Error crítico en limpieza masiva",
            error: error.message
...
// ENDPOINT: GET /cleanup/status - ESTADO GENERAL DE LIMPIEZA
app.get("/cleanup/status", async (req, res) => {
    try {
        const status = {
            ...
            connectingUsers: connectingUsers.size,
            totalUsers: 0,
            usersWithData: 0
        };
        ...
        const usersSnapshot = await firestoreDb.collection('users').get();
        status.totalUsers = usersSnapshot.size;
        
        // Contar usuarios con archivos en el sistema
        const dataDir = path.join(__dirname, "data_v2");
        if (fs.existsSync(dataDir)) {
            const userDirs = fs.readdirSync(dataDir).filter(item => 
                fs.statSync(path.join(dataDir, item)).isDirectory()
            );
            status.usersWithData = userDirs.length;
        }
        ...
        console.error(`[STATUS API] Error obteniendo estado:`, error);
        res.status(500).json({
            success: false,
...
// === FIN FUNCIÓN NUCLEAR DE LIMPIEZA COMPLETA ===
