...
// Variables para almacenar reglas, flujos y disparadores iniciales
let autoReplyRules = [];
let geminiConversationStarters = [];
let actionFlows = [];
let initialTriggers = [];

// <<< AÑADIR: Variables para auto-destrucción de QR >>>
let qrAutoDestroyTimer = null;
let currentQrCode = null;
const QR_AUTO_DESTROY_TIMEOUT = 5 * 60 * 1000; // 5 minutos en milisegundos
// <<< FIN AÑADIR >>>
...
client.on('qr', async (qr) => { // Ya era async
    ...
    try {
        ...
        // <<< AÑADIR: Guardar QR actual y iniciar auto-destrucción >>>
...
client.on('ready', async () => { // <<< ADDED async >>>
    ...
    // <<< AÑADIR: Cancelar auto-destrucción de QR al conectar >>>
    cancelQrAutoDestroy();
    // <<< FIN AÑADIR >>>
    ...
    // Cargar disparadores iniciales para el agente actual
    try {
        initialTriggers = await loadInitialTriggersFromFirestore();
        console.log(`[Worker ${userId}] ${initialTriggers.length} disparadores cargados para agente ${currentAgentState.id || 'default'} al estar ready.`);
    } catch (triggerError) {
        console.error(`[Worker ${userId}] Error cargando disparadores para agente ${currentAgentState.id || 'default'}:`, triggerError);
    }

    // Cargar flujos de acción
    try {
        await loadActionFlowsFromFirestore();
    } catch (flowError) {
        console.error(`[Worker ${userId}] Error cargando flujos de acción:`, flowError);
    }
});
...
client.on('disconnected', async (reason) => { // <<< ADDED async >>>
  ...
  // <<< AÑADIR: Limpieza completa al desconectar >>>
  console.log(`[Worker ${userId}][Disconnect Cleanup] Iniciando limpieza completa...`);
  
  // 1. Cancelar auto-destrucción de QR si está activa
  cancelQrAutoDestroy();
  
  // 2. Limpiar variables locales
  currentQrCode = null;
  
  // 3. Limpiar tracking de tokens de conversación
  if (typeof cleanupOldConversations === 'function') {
    cleanupOldConversations(0); // Limpiar todas las conversaciones inmediatamente
  }
  
  // 4. Limpiar cache de memoria (variables globales)
  autoReplyRules = [];
  geminiConversationStarters = [];
  actionFlows = [];
  initialTriggers = [];
  
  console.log(`[Worker ${userId}][Disconnect Cleanup] Cache de memoria limpiado`);
  // <<< FIN AÑADIR >>>
    ...
    try {
      if (isFromMe) {
        ...
        await chatDocRef.set({
          ...
          lastActivityTimestamp: serverTimestamp // Actualizar timestamp de última actividad
        }, { merge: true });
        ...
        // <<< MODIFICADO: Añadir verificación de activación del chat o trigger inicial >>>
        // Verificar si el chat ya está activado
        const chatIsActivated = await isChatActivated(sender);
        console.log(`[Worker ${userId}][ACTIVATION CHECK] Chat ${sender} activado: ${chatIsActivated ? 'SÍ' : 'NO'}`);
        
        // Verificar si el mensaje es un disparador inicial (solo si el chat no está activado)
        let isInitialTrigger = false;
        if (!chatIsActivated) {
          isInitialTrigger = await isInitialTriggerMessage(message);
          console.log(`[Worker ${userId}][TRIGGER CHECK] Mensaje es disparador inicial: ${isInitialTrigger ? 'SÍ' : 'NO'}`);
          
          // Si es un disparador inicial, activar el chat para futuras interacciones
          if (isInitialTrigger) {
            await activateChat(sender);
          }
        }
        
        // Si el usuario está inactivo Y el bot NO está pausado Y (el chat está activado O el mensaje es un trigger inicial), procesar respuestas automáticas
        if (!userIsActive && !botPauseState && (chatIsActivated || isInitialTrigger)) {
          console.log(`[Worker ${userId}][AUTO-REPLY v2] Usuario INACTIVO, bot NO PAUSADO, y chat ${chatIsActivated ? 'ACTIVADO' : 'activado por TRIGGER'}. Procesando respuestas automáticas para: ${sender}`);
...
process.on('message', async (message) => { // <-- Convertido a async para poder usar await
    ...
    switch (message.type) {
        ...
        case 'SWITCH_AGENT':
            ...
            // <<< RELOAD TRIGGERS FOR THE NEW AGENT >>>
            console.log(`[Worker ${userId}][IPC] Recargando disparadores para el nuevo agente ${currentAgentState.id || 'default'}...`);
            try {
                initialTriggers = await loadInitialTriggersFromFirestore();
                console.log(`[Worker ${userId}][IPC] ${initialTriggers.length} disparadores cargados para el nuevo agente.`);
            } catch (error) {
                console.error(`[Worker ${userId}][IPC] Error cargando disparadores para el nuevo agente:`, error);
                initialTriggers = []; // Clear triggers on error
            }
        ...
        // <<< AÑADIR: Nuevo comando para destruir sesión >>>
        case 'DESTROY_SESSION':
            console.log(`[Worker ${userId}][IPC] Recibido comando DESTROY_SESSION.`);
            await handleSessionDestroy();
            break;
        // <<< FIN AÑADIR >>>
        ...
        case 'RELOAD_AGENT_CONFIG':
            ...
            break;
        case 'RELOAD_INITIAL_TRIGGERS':
            console.log(`[Worker ${userId}][COMMAND] Recargando disparadores iniciales...`);
            // Check if the reload is for the current agent or a different one
            const targetAgentId = message.payload?.agentId;
            if (targetAgentId && targetAgentId !== currentAgentState.id) {
                console.log(`[Worker ${userId}][COMMAND] Reload request for agent ${targetAgentId}, but current agent is ${currentAgentState.id}. Ignoring.`);
                break;
            }
            loadInitialTriggersFromFirestore()
                .then(triggers => {
                    initialTriggers = triggers; // Store the loaded triggers
                    console.log(`[Worker ${userId}][COMMAND] ${triggers.length} disparadores iniciales recargados exitosamente para agente ${currentAgentState.id || 'default'}.`);
                })
                .catch(error => {
                    console.error(`[Worker ${userId}][COMMAND] Error recargando disparadores iniciales:`, error);
                });
            break;
        case 'CHATS_DEACTIVATED':
            console.log(`[Worker ${userId}][COMMAND] Notificación de desactivación de chats recibida.`);
            break;
        default:
...
function handleCommand(command, payload) {
    console.log(`[Worker ${userId}][COMMAND] Procesando comando: ${command}`, payload);
    switch (command) {
        case 'SEND_MESSAGE':
            // ... existing code ...
            break;
        ...
        case 'RELOAD_INITIAL_TRIGGERS':
            console.log(`[Worker ${userId}][COMMAND] Recargando disparadores iniciales...`);
            // Check if the reload is for the current agent or a different one
            const targetAgentIdCmd = payload?.agentId;
            if (targetAgentIdCmd && targetAgentIdCmd !== currentAgentState.id) {
                console.log(`[Worker ${userId}][COMMAND] Reload request for agent ${targetAgentIdCmd}, but current agent is ${currentAgentState.id}. Ignoring.`);
                break;
            }
            loadInitialTriggersFromFirestore()
                .then(triggers => {
        ...
        default:
            console.warn(`[Worker ${userId}][COMMAND] Comando desconocido: ${command}`);
    }
}
...
async function sendBotMessage(chatId, content, type = "Auto") {
    ...
    const serverTimestamp = admin.firestore.FieldValue.serverTimestamp();
    await chatDocRef.set({
      lastBotMessageTimestamp: serverTimestamp,
...
// <<< NUEVA FUNCIÓN: Verificar si un chat está activado >>>
async function isChatActivated(chatId) {
  if (!firestoreDbWorker || !userId) {
    console.error(`[Worker ${userId}][Chat Activation] Firestore DB o User ID no disponibles.`);
    return true; // Por defecto permitir responder si hay error
  }
  ...
  try {
    ...
    // Si el chat no existe o no está activado, retornar false
    if (!chatDoc.exists || chatDoc.data().isActivated !== true) {
      console.log(`[Worker ${userId}][Chat Activation] Chat ${chatId} NO está activado.`);
      return false;
    }
    
    // Verificar si ha pasado el tiempo de inactividad (36 horas)
    const INACTIVITY_TIMEOUT_MS = 36 * 60 * 60 * 1000; // 36 horas
    const lastActivityTimestamp = chatDoc.data().lastActivityTimestamp?.toDate();
    const now = new Date();
    
    // Si no hay timestamp de última actividad, usamos la fecha de activación
    const lastActivity = lastActivityTimestamp || chatDoc.data().activatedAt?.toDate();
    
    if (lastActivity) {
      const inactiveTimeMs = now.getTime() - lastActivity.getTime();
      
      // Si ha pasado más tiempo que el timeout, desactivar el chat silenciosamente
      if (inactiveTimeMs > INACTIVITY_TIMEOUT_MS) {
        console.log(`[Worker ${userId}][Chat Activation] Chat ${chatId} ha estado inactivo por ${Math.round(inactiveTimeMs/1000/60/60)} horas. Desactivando...`);
        
        // Desactivar el chat sin mensaje de despedida
        await deactivateChat(chatId);
        return false;
      }
    }
    
    console.log(`[Worker ${userId}][Chat Activation] Chat ${chatId} está activo y dentro del periodo de actividad.`);
    return true;
  } catch (error) {
    console.error(`[Worker ${userId}][Chat Activation] Error verificando activación:`, error);
    return true; // Por seguridad, permitir responder si hay error
  }
}

// <<< NUEVA FUNCIÓN: Verificar si un mensaje coincide con algún disparador inicial del agente actual >>>
async function isInitialTriggerMessage(message) {
  if (!firestoreDbWorker || !userId) {
    console.error(`[Worker ${userId}][TRIGGER CHECK] Firestore DB o User ID no disponibles.`);
    return false;
  }

  try {
    // Si no hay disparadores configurados en memoria, intentar cargarlos
    if (!initialTriggers || initialTriggers.length === 0) {
      initialTriggers = await loadInitialTriggersFromFirestore();
    }
    
    // Si aún no hay disparadores después de cargar, no hay coincidencia
    if (!initialTriggers || initialTriggers.length === 0) {
      console.log(`[Worker ${userId}][TRIGGER CHECK] No hay disparadores configurados para agente ${currentAgentState.id || 'default'}.`);
      return false;
    }
    ...
    // Usar el array global de disparadores
    for (const trigger of initialTriggers) {
      const triggerText = trigger.text.trim().toLowerCase();
      const triggerType = trigger.type || 'contains'; // Por defecto "contains"
      ...
      if (matches) {
        isTriggered = true;
        triggeredBy = trigger.text;
        break; // Salir del bucle en la primera coincidencia
      }
    }

    if (isTriggered) {
      console.log(`[Worker ${userId}][TRIGGER CHECK] Mensaje coincide con disparador del agente ${currentAgentState.id || 'default'}: "${triggeredBy}"`);
    } else {
      console.log(`[Worker ${userId}][TRIGGER CHECK] Mensaje no coincide con ningún disparador del agente ${currentAgentState.id || 'default'}.`);
    }

    return isTriggered;
  } catch (error) {
    console.error(`[Worker ${userId}][TRIGGER CHECK] Error verificando disparadores para agente ${currentAgentState.id || 'default'}:`, error);
    return false;
  }
}

// <<< NUEVA FUNCIÓN: Activar un chat después de recibir un disparador >>>
async function activateChat(chatId) {
  ...
  try {
    ...
    await chatDocRef.set({
      ...
      lastActivityTimestamp: serverTimestamp, // Establecer timestamp de última actividad
      activationMethod: 'initial_trigger'
    }, { merge: true });
    
    console.log(`[Worker ${userId}][Chat Activation] Chat ${chatId} ACTIVADO exitosamente.`);
    return true;
  } catch (error) {
    console.error(`[Worker ${userId}][Chat Activation] Error activando chat:`, error);
    return false;
  }
}

// <<< NUEVA FUNCIÓN: Cargar disparadores iniciales desde Firestore para el agente actual >>>
async function loadInitialTriggersFromFirestore() {
  if (!firestoreDbWorker || !userId) {
    console.error(`[Worker ${userId}][Initial Triggers Load] Firestore DB o User ID no disponibles.`);
    return [];
  }
  
  // Get the current agent ID, fallback to 'default' if none
  const currentAgentId = currentAgentState.id || 'default';
  console.log(`[Worker ${userId}][Initial Triggers Load] Cargando disparadores desde Firestore para agente: ${currentAgentId}`);
  
  try {
    // Load triggers from the agent-specific collection
    const triggersRef = firestoreDbWorker.collection('users').doc(userId).collection('agents').doc(currentAgentId).collection('triggers');
    const triggersSnapshot = await triggersRef.get();
    
    if (triggersSnapshot.empty) {
      console.log(`[Worker ${userId}][Initial Triggers Load] No hay disparadores configurados para el agente ${currentAgentId}.`);
      return [];
    }
    ...
    console.log(`[Worker ${userId}][Initial Triggers Load] ${triggers.length} disparadores cargados para agente ${currentAgentId}.`);
    return triggers;
  } catch (error) {
    console.error(`[Worker ${userId}][Initial Triggers Load] Error cargando disparadores para agente ${currentAgentId}:`, error);
    return [];
  }
}

// <<< NUEVA FUNCIÓN: Desactivar chat sin mensaje de despedida >>>
async function deactivateChat(chatId) {
  try {
    console.log(`[Worker ${userId}][Chat Deactivation] Desactivando chat ${chatId} por inactividad...`);
    
    // Desactivar el chat en Firestore
    const chatDocRef = firestoreDbWorker.collection('users').doc(userId).collection('chats').doc(chatId);
    await chatDocRef.set({
      isActivated: false,
      deactivatedAt: admin.firestore.FieldValue.serverTimestamp(),
      deactivationReason: 'inactivity_timeout'
    }, { merge: true });
    
    console.log(`[Worker ${userId}][Chat Deactivation] Chat ${chatId} desactivado exitosamente.`);
    return true;
  } catch (error) {
    console.error(`[Worker ${userId}][Chat Deactivation] Error desactivando chat:`, error);
    return false;
  }
}

// <<< AÑADIR: Funciones para auto-destrucción de QR >>>
function startQrAutoDestroy() {
    // Limpiar timer anterior si existe
    if (qrAutoDestroyTimer) {
        clearTimeout(qrAutoDestroyTimer);
        qrAutoDestroyTimer = null;
    }
    
    console.log(`[Worker ${userId}][QR Auto-Destroy] Iniciando timer de auto-destrucción (${QR_AUTO_DESTROY_TIMEOUT/1000}s)`);
    
    qrAutoDestroyTimer = setTimeout(async () => {
        console.log(`[Worker ${userId}][QR Auto-Destroy] QR no utilizado después de ${QR_AUTO_DESTROY_TIMEOUT/1000}s. Destruyendo...`);
        await destroyUnusedQr();
    }, QR_AUTO_DESTROY_TIMEOUT);
}

async function destroyUnusedQr() {
    try {
        console.log(`[Worker ${userId}][QR Auto-Destroy] Destruyendo QR no utilizado...`);
        ...
        await updateFirestoreStatus({ 
            ...
            qrCodeUrl: null, 
            error: null, 
            message: 'QR code expired due to inactivity' 
        });
        
        // Notificar al servidor
        sendStatusUpdate('qr_expired', 'QR code expired due to inactivity');
        
        console.log(`[Worker ${userId}][QR Auto-Destroy] QR destruido exitosamente`);
        
    } catch (error) {
        console.error(`[Worker ${userId}][QR Auto-Destroy] Error destruyendo QR:`, error);
        sendErrorInfo(`Error destroying expired QR: ${error.message}`);
    }
}

function cancelQrAutoDestroy() {
    if (qrAutoDestroyTimer) {
        clearTimeout(qrAutoDestroyTimer);
        qrAutoDestroyTimer = null;
        console.log(`[Worker ${userId}][QR Auto-Destroy] Timer de auto-destrucción cancelado`);
    }
}
// <<< FIN AÑADIR >>>

// <<< AÑADIR: Función para manejar destrucción de sesión >>>
async function handleSessionDestroy() {
    try {
        console.log(`[Worker ${userId}][Session Destroy] Iniciando destrucción de sesión...`);
        
        // 1. Cancelar auto-destrucción de QR
        cancelQrAutoDestroy();
        
        // 2. Limpiar variables locales
        currentQrCode = null;
        
        // 3. Limpiar cache de memoria
        autoReplyRules = [];
        geminiConversationStarters = [];
        actionFlows = [];
        initialTriggers = [];
        
        // 4. Limpiar tracking de conversaciones
        if (typeof cleanupOldConversations === 'function') {
            cleanupOldConversations(0);
        }
        
        // 5. Intentar destruir la sesión de WhatsApp Web si el cliente está disponible
        if (client) {
            try {
                await client.destroy();
                console.log(`[Worker ${userId}][Session Destroy] Cliente WhatsApp destruido`);
            } catch (clientError) {
                console.error(`[Worker ${userId}][Session Destroy] Error destruyendo cliente:`, clientError);
            }
        }
        ...
        await updateFirestoreStatus({ 
            ...
            qrCodeUrl: null, 
            error: null, 
            message: 'Session destroyed by user request' 
        });
        
        // 7. Notificar al servidor
        sendStatusUpdate('session_destroyed', 'Session destroyed by user request');
        
        console.log(`[Worker ${userId}][Session Destroy] Sesión destruida exitosamente`);
        
        // 8. Terminar el proceso worker después de un breve delay
        setTimeout(() => {
            console.log(`[Worker ${userId}][Session Destroy] Terminando proceso worker...`);
            process.exit(0);
        }, 1000);
        
    } catch (error) {
        console.error(`[Worker ${userId}][Session Destroy] Error destruyendo sesión:`, error);
        sendErrorInfo(`Error destroying session: ${error.message}`);
